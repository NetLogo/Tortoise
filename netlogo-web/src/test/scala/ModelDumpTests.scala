...
class ModelDumpTests extends FunSuite {
  val engineSource = io.Source.fromFile("netlogo-web/target/resource_managed/tortoise.js").mkString
  ...
  val compilationFunction: Array[AnyRef] => AnyRef = {
    ...
    rhinoEngine.function(
      "function(s) { return (new BrowserCompiler()).fromNlogo(s); }")
  }
  ...
  for (path <- Model.models.map(_.path).distinct) {
    test(s"outputs correct model javascript for ${path}") {
      ...
      try {
        ...
        assert(compilationResult.get("code").toString.contains("to"))
        if (! path.contains("benchmark"))
          assert(compilationResult.get("info").toString.contains("WHAT IS IT"))
      } catch {
