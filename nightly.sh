...
./sbt 'set logBuffered in test := false' tortoiseJsTest/test 2>&1 | tee tmp/nightly/slow-test.txt
if [ ${PIPESTATUS[0]} -ne 0 ] ; then echo "*** FAILED: tortoiseJsTest/test"; exit 1; fi
echo "*** done: tortoiseJsTest/test"
