...
var procedures = (function() {
  ...
  temp = (function() {
    try {
      ...
      Errors.askNobodyCheck(PrimChecks.list.nOf(PrimChecks.validator.checkArg('N-OF', 1, world.observer.getGlobal("hive-number")), PrimChecks.agentset.with(world.patches(), function() {
      ...
      for (let _index_3202_3208 = 0, _repeatcount_3202_3208 = StrictMath.floor(PrimChecks.agentset.count(world.turtleManager.turtlesOfBreed("SITES"))); _index_3202_3208 < _repeatcount_3202_3208; _index_3202_3208++){
        ...
        plotManager.setPenColor(PrimChecks.list.item(PrimChecks.validator.checkArg('ITEM', 1, i), PrimChecks.validator.checkArg('ITEM', 12, world.observer.getGlobal("color-list"))));
        i = PrimChecks.math.plus(PrimChecks.validator.checkArg('+', 1, i), 1); letVars['i'] = i;
      }
    } catch (e) {
      return Errors.stopInCommandCheck(e)
    }
  ...
  temp = (function() {
    try {
      ...
      Errors.askNobodyCheck(PrimChecks.list.nOf(PrimChecks.validator.checkArg('N-OF', 1, world.observer.getGlobal("initial-percentage")), world.turtleManager.turtlesOfBreed("SCOUTS"))).ask(function() {
      ...
      world.observer.setGlobal("discover-task", Tasks.commandTask(function() {
        if (Prims.lt(SelfManager.self().getVariable("bee-timer"), 0)) {
          ...
          if (PrimChecks.agentset.optimizeCount(SelfManager.self().inRadius(world.turtleManager.turtlesOfBreed("SITES"), 3), 0, (a, b) => a > b)) {
            ...
            if (PrimChecks.math.not(PrimChecks.validator.checkArg('NOT', 2, PrimChecks.agentset.of(PrimChecks.validator.checkArg('OF', 1904, tempTarget), function() { return SelfManager.self().getVariable("discovered?"); })))) {
              ...
              Errors.askNobodyCheck(SelfManager.self().getVariable("target")).ask(function() {
                ...
                SelfManager.self().setVariable("color", PrimChecks.list.item(PrimChecks.validator.checkArg('ITEM', 1, SelfManager.self().getVariable("who")), PrimChecks.validator.checkArg('ITEM', 12, world.observer.getGlobal("color-list"))));
              }, true);
    ...
    try {
      ...
      world.observer.setGlobal("inspect-hive-task", Tasks.commandTask(function() {
        if (Prims.lt(SelfManager.self().getVariable("bee-timer"), 0)) {
          ...
          if (Prims.gt(PrimChecks.agentset.count(PrimChecks.validator.checkArg('COUNT', 112, nearbyScouts)), world.observer.getGlobal("quorum"))) {
            SelfManager.self().setVariable("next-task", world.observer.getGlobal("go-home-task"));
            SelfManager.self().setVariable("task-string", "going-home");
            SelfManager.self().setVariable("on-site?", false);
            SelfManager.self().setVariable("piping?", true);
  ...
  temp = (function() {
    try {
      var reporterContext = false;
      var letVars = { };
      let numOfTurns = PrimChecks.math.mult(PrimChecks.math.div(1, PrimChecks.validator.checkArg('/', 1, SelfManager.self().getVariable("interest"))), 2600); letVars['numOfTurns'] = numOfTurns;
      let anglePerTurn = PrimChecks.math.div(180, PrimChecks.validator.checkArg('/', 1, numOfTurns)); letVars['anglePerTurn'] = anglePerTurn;
      let semicircle = PrimChecks.math.div(PrimChecks.math.mult(PrimChecks.math.mult(0.5, PrimChecks.validator.checkArg('*', 1, SelfManager.self().getVariable("dist-to-hive"))), 3.141592653589793), 5); letVars['semicircle'] = semicircle;
      if (Prims.equality(SelfManager.self().getVariable("circle-switch"), 1)) {
        ...
        for (let _index_17080_17086 = 0, _repeatcount_17080_17086 = StrictMath.floor(numOfTurns); _index_17080_17086 < _repeatcount_17080_17086; _index_17080_17086++){
          ...
          SelfManager.self().fd(PrimChecks.math.mult(PrimChecks.math.div(PrimChecks.validator.checkArg('/', 1, semicircle), 180), PrimChecks.validator.checkArg('*', 1, anglePerTurn)));
        }
      }
      if (Prims.equality(SelfManager.self().getVariable("circle-switch"), -1)) {
        SelfManager.self().face(SelfManager.self().getVariable("target"));
  ...
  temp = (function() {
    try {
      ...
      for (let _index_17897_17903 = 0, _repeatcount_17897_17903 = StrictMath.floor(PrimChecks.math.div(PrimChecks.math.minus(PrimChecks.validator.checkArg('-', 1, SelfManager.self().getVariable("dist-to-hive")), 2), 2)); _index_17897_17903 < _repeatcount_17897_17903; _index_17897_17903++){
        ...
        waggleSwitch = PrimChecks.math.mult(PrimChecks.validator.checkArg('*', 1, waggleSwitch), -1); letVars['waggleSwitch'] = waggleSwitch;
      }
      if (Prims.equality(waggleSwitch, -1)) {
        SelfManager.self().right(-(120));
        SelfManager.self()._optimalFdLessThan1(0.4);
