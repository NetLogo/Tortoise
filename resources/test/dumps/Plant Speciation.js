        ...
        try {
          ...
          if ((PrimChecks.agentset.anyWith(world.turtles(), function() {
            ...
            for (let _index_215_221 = 0, _repeatcount_215_221 = StrictMath.floor(500); _index_215_221 < _repeatcount_215_221; _index_215_221++){
              ...
              if (Prims.lt(PrimChecks.math.abs((PrimChecks.agentset.of(r, function() { return SelfManager.self().getVariable("flower-time"); }) - PrimChecks.agentset.of(s, function() { return SelfManager.self().getVariable("flower-time"); }))), world.observer.getGlobal("flower-duration"))) {
                m = (m + 1); letVars['m'] = m;
              }
              n = (n + 1); letVars['n'] = n;
            }
            world.observer.setGlobal("percent-same-flowering-time", PrimChecks.math.precision((PrimChecks.math.div(m, n) * 100), 1));
...
var procedures = (function() {
  ...
  temp = (function() {
    try {
      var reporterContext = false;
      var letVars = { };
      SelfManager.self().setPatchVariable("metal", PrimChecks.math.precision(PrimChecks.math.div(100, (1 + PrimChecks.math.exp((world.observer.getGlobal("frontier-sharpness") * (PrimChecks.math.div((world.topology.maxPxcor + world.topology.minPxcor), 2) - SelfManager.self().getPatchVariable("pxcor")))))), 0));
    } catch (e) {
      return Errors.stopInCommandCheck(e)
    }
  });
      ...
      Errors.askNobodyCheck(world.turtles()).ask(function() {
        ...
        SelfManager.self().hatch(PrimChecks.math.randomPoisson(world.observer.getGlobal("average-number-offspring")), "").ask(function() {
          ...
          if (Prims.equality(world.observer.getGlobal("genetics-model"), "avg. genotype")) {
            SelfManager.self().setVariable("tolerance", PrimChecks.math.div((SelfManager.self().getVariable("tolerance") + PrimChecks.agentset.of(mate, function() { return SelfManager.self().getVariable("tolerance"); })), 2));
            SelfManager.self().setVariable("flower-time", PrimChecks.math.div((SelfManager.self().getVariable("flower-time") + PrimChecks.agentset.of(mate, function() { return SelfManager.self().getVariable("flower-time"); })), 2));
          }
  ...
  temp = (function() {
    try {
      ...
      Errors.askNobodyCheck(world.patches()).ask(function() {
        let overpopulation = (PrimChecks.agentset.count_unchecked(SelfManager.self().turtlesHere()) - world.observer.getGlobal("plants-per-patch")); letVars['overpopulation'] = overpopulation;
  ...
  temp = (function(options, weights) {
    try {
      ...
      let wret = (wsum * PrimChecks.math.randomFloat(1)); letVars['wret'] = wret;
  ...
  temp = (function(t1, t2) {
    try {
      var reporterContext = true;
      var letVars = { };
      let diff = PrimChecks.math.abs((PrimChecks.agentset.of(t1, function() { return SelfManager.self().getVariable("flower-time"); }) - PrimChecks.agentset.of(t2, function() { return SelfManager.self().getVariable("flower-time"); }))); letVars['diff'] = diff;
