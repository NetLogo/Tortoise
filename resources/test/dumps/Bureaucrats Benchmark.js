    ...
    return workspace.rng.withClone(function() {
      return plotManager.withTemporaryContext('Average', 'average')(function() {
        try {
          ...
          var letVars = { };
          plotManager.plotPoint(world.ticker.tickCount(), PrimChecks.math.div(world.observer.getGlobal("total"), PrimChecks.agentset.count_unchecked(world.patches())));
        } catch (e) {
...
var procedures = (function() {
  ...
  temp = (function() {
    try {
      var reporterContext = false;
      var letVars = { };
      PrimChecks.math.randomSeed_unchecked(0);
  ...
  temp = (function() {
    try {
      ...
      world.observer.setGlobal("total", PrimChecks.math.mult_unchecked(2, PrimChecks.agentset.count_unchecked(world.patches())));
      world.ticker.reset();
    } catch (e) {
      return Errors.stopInCommandCheck(e)
    }
  ...
  temp = (function() {
    try {
      var reporterContext = false;
      var letVars = { };
      let activePatches = PrimChecks.agentset.patchSet(PrimChecks.list.oneOf_unchecked(world.patches())); letVars['activePatches'] = activePatches;
      Errors.askNobodyCheck(activePatches).ask(function() {
        SelfManager.self().setPatchVariable("n", PrimChecks.math.plus(SelfManager.self().getPatchVariable("n"), 1));
        world.observer.setGlobal("total", PrimChecks.math.plus(world.observer.getGlobal("total"), 1));
        procedures["COLORIZE"]();
      }, true);
      while (PrimChecks.agentset.any(activePatches)) {
        ...
        Errors.askNobodyCheck(overloadedPatches).ask(function() {
          SelfManager.self().setPatchVariable("n", PrimChecks.math.minus(SelfManager.self().getPatchVariable("n"), 4));
          world.observer.setGlobal("total", PrimChecks.math.minus(world.observer.getGlobal("total"), 4));
          procedures["COLORIZE"]();
