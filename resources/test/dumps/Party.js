...
function setup() {
  ...
  AgentSet.ask(world.createTurtles(Globals.getGlobal(1), ""), true, function() {
    ...
    AgentSet.setTurtleVariable(14, AgentSet.oneOf(Globals.getGlobal(3)));
...
function go() {
  if (AgentSet.all(world.turtles(), function(){ return AgentSet.getTurtleVariable(13) })) {
  ...
  Call(countBoringGroups);
  AgentSet.ask(world.turtles(), true, function() {
    AgentSet.setTurtleVariable(14, AgentSet.self().getPatchHere());
    Call(spreadOutVertically);
  });
  world.tick();
}
function updateHappiness() {
  ...
  AgentSet.setTurtleVariable(13, Prims.lte((opposite / total), (Globals.getGlobal(0) / 100)));
}
function leaveIfUnhappy() {
  if (!(AgentSet.getTurtleVariable(13))) {
    AgentSet.setTurtleVariable(2, AgentSet.oneOf([90, 270]));
    Prims.fd(1);
  }
}
...
function groupSite_p() {
  var groupInterval = StrictMath.floor((world.width() / Globals.getGlobal(2)));
  return (((Prims.equality(AgentSet.getPatchVariable(1), 0) && Prims.lte(AgentSet.getPatchVariable(0), 0)) && Prims.equality(Prims.mod(AgentSet.getPatchVariable(0), groupInterval), 0)) && Prims.lt(StrictMath.floor(((- AgentSet.getPatchVariable(0)) / groupInterval)), Globals.getGlobal(2)));
}
function spreadOutVertically() {
  ...
  while (AgentSet.any(AgentSet.other(AgentSet.self().turtlesHere()))) {
    if (AgentSet.self().canMove(2)) {
      ...
      AgentSet.setTurtleVariable(3, (AgentSet.getTurtleVariable(3) - 1));
      AgentSet.setTurtleVariable(4, 0);
      Prims.fd(4);
    }
  }
}
function countBoringGroups() {
  AgentSet.ask(Globals.getGlobal(3), true, function() {
    if (Call(boring_p)) {
  ...
  Globals.setGlobal(4, AgentSet.count(AgentSet.agentFilter(Globals.getGlobal(3), function() {
...
function chooseSex() {
  AgentSet.setTurtleVariable(1, AgentSet.oneOf([135, 105]));
}
function woman_p() {
  return Prims.equality(AgentSet.getTurtleVariable(1), 135);
}
Globals.setGlobal(0, 25);
Globals.setGlobal(1, 70);
Globals.setGlobal(2, 10);
