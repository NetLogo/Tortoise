...
function setup() {
  ...
  AgentSet.ask(world.createTurtles(world.observer.getGlobal('number'), ""), true, function() {
    ...
    AgentSet.setTurtleVariable(14, AgentSet.oneOf(world.observer.getGlobal('group-sites')));
...
function groupSite_p() {
  ...
  return (((Prims.equality(AgentSet.getPatchVariable(1), 0) && Prims.lte(AgentSet.getPatchVariable(0), 0)) && Prims.equality(Prims.mod(AgentSet.getPatchVariable(0), groupInterval), 0)) && Prims.lt(StrictMath.floor(((- AgentSet.getPatchVariable(0)) / groupInterval)), world.observer.getGlobal('num-groups')));
}
