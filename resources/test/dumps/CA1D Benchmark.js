...
var procedures = (function() {
  ...
  var go = function() {
    try {
      ...
      world.patches().agentFilter(function() {
        return Prims.equality(SelfManager.self().getPatchVariable("pycor"), world.observer.getGlobal("row"));
      }).ask(function() { procedures.doRule(); }, true);
      world.observer.setGlobal("row", (world.observer.getGlobal("row") - 1));
      world.patches().agentFilter(function() {
        return Prims.equality(SelfManager.self().getPatchVariable("pycor"), world.observer.getGlobal("row"));
      }).ask(function() { procedures.colorPatch(); }, true);
      world.observer.setGlobal("gone?", true);
      world.ticker.tick();
    } catch (e) {
      if (e instanceof Exception.StopInterrupt) {
  ...
  var showRules = function() {
    ...
    world.patches().agentFilter(function() {
      ...
      SelfManager.self().sprout(1, "TURTLES").ask(function() {
        ...
        procedures.printBlock(ListPrims.item(0, ListPrims.item(SelfManager.self().getVariable("who"), rules)));
        SelfManager.self().fd(2);
        procedures.printBlock(ListPrims.item(1, ListPrims.item(SelfManager.self().getVariable("who"), rules)));
        SelfManager.self().fd(2);
        procedures.printBlock(ListPrims.item(2, ListPrims.item(SelfManager.self().getVariable("who"), rules)));
        SelfManager.self().fd(-2);
        SelfManager.self().setVariable("heading", 180);
        SelfManager.self().fd(2);
        SelfManager.self().setVariable("heading", 90);
        procedures.printBlock(ListPrims.item(3, ListPrims.item(SelfManager.self().getVariable("who"), rules)));
        SelfManager.self().die();
      }, true);
    }, true);
    world.observer.setGlobal("rules-shown?", true);
  };
  var printBlock = function(state) {
    if (state) {
  ...
  return {
    "BENCHMARK":benchmark,
    "BINDIGIT":bindigit,
    "CALCULATE-RULE":calculateRule,
    "COLOR-PATCH":colorPatch,
    "DO-RULE":doRule,
    "EXTRAPOLATE-SWITCHES":extrapolateSwitches,
    "GO":go,
    ...
    "doRule":doRule,
    "extrapolateSwitches":extrapolateSwitches,
    "go":go,
    "listRules":listRules,
