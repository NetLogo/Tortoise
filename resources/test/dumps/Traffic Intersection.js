...
function setup() {
  ...
  world.patches().ask(function() {
    SelfPrims.setPatchVariable('pcolor', (55 - 1));
    if ((Prims.lte(StrictMath.abs(SelfPrims.getPatchVariable('pxcor')), 1) || Prims.lte(StrictMath.abs(SelfPrims.getPatchVariable('pycor')), 1))) {
      SelfPrims.setPatchVariable('pcolor', 0);
    }
  }, true);
  world.observer.setGlobal('stop-light', "north");
  Call(drawStopLight);
  world.ticker.reset();
}
function drawStopLight() {
  world.patches().agentFilter(function() {
    return (Prims.lte(StrictMath.abs(SelfPrims.getPatchVariable('pxcor')), 1) && Prims.lte(StrictMath.abs(SelfPrims.getPatchVariable('pycor')), 1));
  }).ask(function() {
    SelfPrims.setPatchVariable('pcolor', 0);
  }, true);
  if (Prims.equality(world.observer.getGlobal('stop-light'), "north")) {
    world.getPatchAt(0, -1).ask(function() {
      SelfPrims.setPatchVariable('pcolor', 15);
    }, true);
    world.getPatchAt(-1, 0).ask(function() {
...
function go() {
  Call(moveCars);
  Call(makeNewCars);
  if (world.observer.getGlobal('auto?')) {
    if (Prims.equality(Prims.mod(world.ticker.tickCount(), (world.observer.getGlobal('green-length') + world.observer.getGlobal('yellow-length'))), 0)) {
      Call(_switch_);
    }
    if (Prims.gt(Prims.mod(world.ticker.tickCount(), (world.observer.getGlobal('green-length') + world.observer.getGlobal('yellow-length'))), world.observer.getGlobal('green-length'))) {
      world.patches().agentFilter(function() {
...
function makeNewCars() {
  if ((Prims.lt(Prims.randomFloat(100), world.observer.getGlobal('freq-north')) && !Prims.turtlesOn(world.getPatchAt(0, world.topology.minPycor)).nonEmpty())) {
    world.turtleManager.createTurtles(1, '').ask(function() {
      SelfPrims.setVariable('ycor', world.topology.minPycor);
      SelfPrims.setVariable('heading', 0);
      SelfPrims.setVariable('color', (5 + (10 * Prims.random(14))));
      SelfPrims.setVariable('speed', ListPrims.min(ListPrims.list(Call(clearAhead), world.observer.getGlobal('speed-limit'))));
    }, true);
  }
  if ((Prims.lt(Prims.randomFloat(100), world.observer.getGlobal('freq-east')) && !Prims.turtlesOn(world.getPatchAt(world.topology.minPxcor, 0)).nonEmpty())) {
    world.turtleManager.createTurtles(1, '').ask(function() {
      SelfPrims.setVariable('xcor', world.topology.minPxcor);
...
function move() {
  var clearTo = Call(clearAhead);
  if (Prims.gt(clearTo, SelfPrims.getVariable('speed'))) {
    if (Prims.lt(SelfPrims.getVariable('speed'), world.observer.getGlobal('speed-limit'))) {
      SelfPrims.setVariable('speed', (SelfPrims.getVariable('speed') + ListPrims.min(ListPrims.list(world.observer.getGlobal('max-accel'), ((clearTo - 1) - SelfPrims.getVariable('speed'))))));
    }
    ...
    SelfPrims.setVariable('speed', (SelfPrims.getVariable('speed') - ListPrims.min(ListPrims.list(world.observer.getGlobal('max-brake'), (SelfPrims.getVariable('speed') - (clearTo - 1))))));
    if (Prims.lt(SelfPrims.getVariable('speed'), 0)) {
      SelfPrims.setVariable('speed', 0);
    }
  }
  Prims.repeat(SelfPrims.getVariable('speed'), function() {
    SelfPrims.fd(1);
    if (!SelfManager.self().canMove(1)) {
      SelfPrims.die();
    }
    if (Prims.equality(SelfPrims.getPatchVariable('pcolor'), 25)) {
      SelfPrims.setPatchVariable('clear-in', 5);
      SelfPrims.die();
    }
  });
}
function clearAhead() {
  var n = 1;
  Prims.repeat((world.observer.getGlobal('max-accel') + SelfPrims.getVariable('speed')), function() {
    if ((Prims.lte(((n * SelfManager.self().dx()) + SelfPrims.getPatchVariable('pxcor')), world.topology.maxPxcor) && Prims.lte(((n * SelfManager.self().dy()) + SelfPrims.getPatchVariable('pycor')), world.topology.maxPycor))) {
      if (((Prims.equality(SelfManager.self().patchAhead(n).projectionBy(function() {
        return SelfPrims.getPatchVariable('pcolor');
      }), 15) || Prims.equality(SelfManager.self().patchAhead(n).projectionBy(function() {
        return SelfPrims.getPatchVariable('pcolor');
      }), 25)) || Prims.turtlesOn(SelfManager.self().patchAhead(n)).nonEmpty())) {
        return n;
      }
      n = (n + 1);
    }
...
function checkForCollisions() {
  world.patches().agentFilter(function() {
    ...
    SelfPrims.setPatchVariable('clear-in', (SelfPrims.getPatchVariable('clear-in') - 1));
    if (Prims.equality(SelfPrims.getPatchVariable('clear-in'), 0)) {
      SelfPrims.setPatchVariable('pcolor', 0);
    }
  }, true);
  world.patches().agentFilter(function() {
    return Prims.gt(SelfManager.self().turtlesHere().size(), 1);
  }).ask(function() {
    SelfPrims.setPatchVariable('pcolor', 25);
    SelfPrims.setPatchVariable('clear-in', 5);
...
function _switch_() {
  if (Prims.equality(world.observer.getGlobal('stop-light'), "north")) {
    world.observer.setGlobal('stop-light', "east");
    Call(drawStopLight);
  }
  else {
    world.observer.setGlobal('stop-light', "north");
...
world.observer.setGlobal('green-length', 12);
world.observer.setGlobal('speed-limit', 5);
world.observer.setGlobal('max-accel', 2);
world.observer.setGlobal('max-brake', 4);
world.observer.setGlobal('freq-north', 60);
world.observer.setGlobal('freq-east', 100);
world.observer.setGlobal('auto?', true);
world.observer.setGlobal('yellow-length', 3);
