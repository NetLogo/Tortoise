...
function setupWorld() {
  ...
  AgentSet.ask(world.patches(), true, function() {
    if (Prims.gt(AgentSet.getPatchVariable(1), world.observer.getGlobal('sky-top'))) {
      AgentSet.setPatchVariable(2, Prims.scaleColor(9.9, AgentSet.getPatchVariable(1), 22, 15));
    }
    if ((Prims.lte(AgentSet.getPatchVariable(1), world.observer.getGlobal('sky-top')) && Prims.gt(AgentSet.getPatchVariable(1), world.observer.getGlobal('earth-top')))) {
      AgentSet.setPatchVariable(2, Prims.scaleColor(105, AgentSet.getPatchVariable(1), -20, 20));
    }
    if (Prims.lt(AgentSet.getPatchVariable(1), world.observer.getGlobal('earth-top'))) {
      AgentSet.setPatchVariable(2, (15 + 3));
    }
    if (Prims.equality(AgentSet.getPatchVariable(1), world.observer.getGlobal('earth-top'))) {
      Call(updateAlbedo);
    }
  });
}
function go() {
  ...
  AgentSet.ask(AgentSet.agentFilter(world.patches(), function() {
    return Prims.equality(AgentSet.getPatchVariable(1), world.observer.getGlobal('earth-top'));
  }), true, function() {
    Call(updateAlbedo);
  });
  Call(runHeat);
...
function updateAlbedo() {
  AgentSet.setPatchVariable(2, Prims.scaleColor(55, world.observer.getGlobal('albedo'), 0, 1));
}
function addCloud() {
