...
function go() {
  ...
  world.topology().diffuse(5, (world.observer.getGlobal('diffusion-rate') / 100));
  AgentSet.ask(world.patches(), true, function() {
    Call(goPatches);
  });
  world.ticker.tick();
...
function goPatches() {
  AgentSet.setPatchVariable(5, ((AgentSet.getPatchVariable(5) * (100 - world.observer.getGlobal('evaporation-rate'))) / 100));
  Call(updateDisplay);
}
function returnToNest() {
  if (AgentSet.getPatchVariable(7)) {
    ...
    AgentSet.setPatchVariable(5, (AgentSet.getPatchVariable(5) + AgentSet.getTurtleVariable('drop-size')));
