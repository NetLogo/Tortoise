...
function setupField() {
  AgentSet.ask(world.patches(), true, function() {
    ...
    if (((Prims.equality(AgentSet.getPatchVariable(1), world.minPycor) && Prims.gte(AgentSet.getPatchVariable(0), world.observer.getGlobal('goal-pos'))) && Prims.lt(AgentSet.getPatchVariable(0), (world.observer.getGlobal('goal-pos') + world.observer.getGlobal('goal-size'))))) {
      AgentSet.setPatchVariable(2, 55);
    }
  });
  world.observer.setGlobal('try-line', AgentSet.agentFilter(world.patches(), function() {
    return (Prims.equality(AgentSet.getPatchVariable(0), world.observer.getGlobal('kick-line')) && Prims.equality(AgentSet.getPatchVariable(2), 0));
  }));
  AgentSet.ask(world.observer.getGlobal('try-line'), true, function() {
    AgentSet.setPatchVariable(2, 45);
  });
  world.observer.setGlobal('histogram-area', AgentSet.agentFilter(world.patches(), function() {
    return (Prims.lt(AgentSet.getPatchVariable(0), world.observer.getGlobal('kick-line')) && Prims.equality(AgentSet.getPatchVariable(2), 0));
  }));
}
function setupBalls() {
...
function plotScores() {
  world.observer.setGlobal('current-max', Prims.max(AgentSet.of(world.observer.getGlobal('try-line'), function() {
    return AgentSet.getPatchVariable(5);
  })));
  if (Prims.equality(world.observer.getGlobal('current-max'), 0)) {
    AgentSet.ask(world.observer.getGlobal('histogram-area'), true, function() {
      AgentSet.setPatchVariable(2, 0);
    });
    throw new Exception.StopInterrupt;
  }
  AgentSet.ask(world.observer.getGlobal('try-line'), true, function() {
    if (Prims.equality(AgentSet.getPatchVariable(5), world.observer.getGlobal('current-max'))) {
      world.observer.setGlobal('best-dist', AgentSet.getPatchVariable(1));
  ...
  AgentSet.ask(world.observer.getGlobal('histogram-area'), true, function() {
    if (Prims.gt(AgentSet.getPatchVariable(0), (world.observer.getGlobal('kick-line') - ((AgentSet.of(AgentSet.self().patchAt((world.observer.getGlobal('kick-line') - AgentSet.getPatchVariable(0)), 0), function() {
      return AgentSet.getPatchVariable(5);
    }) * (world.observer.getGlobal('kick-line') - world.minPxcor)) / world.observer.getGlobal('current-max'))))) {
      if (Prims.equality(AgentSet.of(AgentSet.self().patchAt((world.observer.getGlobal('kick-line') - AgentSet.getPatchVariable(0)), 0), function() {
        return AgentSet.getPatchVariable(5);
...
function findAnalyticSolution() {
  ...
  var winningPatch = AgentSet.minOneOf(world.observer.getGlobal('try-line'), function() {
...
function drawLevelCurves() {
  AgentSet.ask(AgentSet.agentFilter(world.patches(), function() {
    return (Prims.gt(AgentSet.getPatchVariable(0), world.observer.getGlobal('kick-line')) && Prims.lt(AgentSet.getPatchVariable(2), 10));
  }), true, function() {
...
function calcGoalAngle() {
  AgentSet.setPatchVariable(6, AgentSet.self().towardsXY((world.observer.getGlobal('goal-pos') - 0.5), (world.minPycor + 0.5)));
  AgentSet.setPatchVariable(7, AgentSet.self().towardsXY(((world.observer.getGlobal('goal-pos') + world.observer.getGlobal('goal-size')) - 0.5), (world.minPycor + 0.5)));
