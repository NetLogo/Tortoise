...
modelConfig.plots = [(function() {
      ...
      return plotManager.withTemporaryContext('# of bacteria for each variation', undefined)(function() {
        try {
          ...
          var _foreach_153_160 = Tasks.forEach(Tasks.commandTask(function(thisVariation) {
            ...
            plotManager.plotPoint(thisVariation, PrimChecks.agentset.countWith(world.turtleManager.turtlesOfBreed("BACTERIA"), function() { return Prims.equality(SelfManager.self().getVariable("variation"), thisVariation); }));
  ...
  return new Plot(name, pens, plotOps, "# of flagella", "# of bacteria", false, true, 0, 8, 0, 10, setup, update);
})(), (function() {
      ...
      return plotManager.withTemporaryContext('Avg. # of flagella per bacteria', 'pen 1')(function() {
        try {
          ...
          if (PrimChecks.agentset.any(world.turtleManager.turtlesOfBreed("BACTERIA"))) {
            plotManager.plotPoint(world.ticker.tickCount(), PrimChecks.list.mean(PrimChecks.validator.checkArg('MEAN', 8, PrimChecks.agentset.of(world.turtleManager.turtlesOfBreed("BACTERIA"), function() { return SelfManager.self().getVariable("variation"); }))));
          }
        } catch (e) {
          return Errors.stopInCommandCheck(e)
        };
...
var procedures = (function() {
  ...
  temp = (function() {
    try {
      ...
      Errors.askNobodyCheck(world.turtleManager.turtlesOfBreed("BACTERIA")).ask(function() {
        ...
        SelfManager.self().fd(PrimChecks.math.mult(PrimChecks.validator.checkArg('*', 1, SelfManager.self().getVariable("variation")), PrimChecks.validator.checkArg('*', 1, world.observer.getGlobal("speed-scalar"))));
        ...
        if ((PrimChecks.math.not(PrimChecks.validator.checkArg('NOT', 2, world.observer.getGlobal("camouflage?"))) && PrimChecks.agentset.any(PrimChecks.validator.checkArg('ANY?', 112, predatorsInFrontOfMe)))) {
          SelfManager.self().face(PrimChecks.list.oneOf(PrimChecks.validator.checkArg('ONE-OF', 120, predatorsInFrontOfMe)));
  ...
  temp = (function() {
    try {
      ...
      let newSwing = PrimChecks.math.mult(PrimChecks.validator.checkArg('*', 1, flagellaSwing), PrimChecks.math.sin(PrimChecks.math.mult(PrimChecks.validator.checkArg('*', 1, flagellaSpeed), world.ticker.tickCount()))); letVars['newSwing'] = newSwing;
      let myBacteria = PrimChecks.list.oneOf(LinkPrims.inLinkNeighbors("LINKS")); letVars['myBacteria'] = myBacteria;
      SelfManager.self().setVariable("heading", PrimChecks.math.plus(PrimChecks.validator.checkArg('+', 1, PrimChecks.agentset.of(PrimChecks.validator.checkArg('OF', 1904, myBacteria), function() { return SelfManager.self().getVariable("heading"); })), PrimChecks.validator.checkArg('+', 1, newSwing)));
    } catch (e) {
      return Errors.stopInCommandCheck(e)
    }
  });
  ...
  temp = (function() {
    try {
      ...
      let prey = PrimChecks.agentset.of(PrimChecks.validator.checkArg('OF', 1904, PrimChecks.list.oneOf(world.turtleManager.turtlesOfBreed("PREDATORS"))), function() {
        return SelfManager.self().inRadius(world.turtleManager.turtlesOfBreed("BACTERIA"), PrimChecks.math.div(PrimChecks.validator.checkArg('/', 1, SelfManager.self().getVariable("size")), 2));
      }); letVars['prey'] = prey;
      if (PrimChecks.math.not(PrimChecks.agentset.any(PrimChecks.validator.checkArg('ANY?', 112, prey)))) {
        throw new Exception.StopInterrupt;
      }
      Errors.askNobodyCheck(PrimChecks.list.oneOf(PrimChecks.validator.checkArg('ONE-OF', 120, prey))).ask(function() { procedures["DEATH"](); }, true);
    ...
    try {
      ...
      Errors.askNobodyCheck(world.turtleManager.turtlesOfBreed("BACTERIA")).ask(function() {
        ...
        if (PrimChecks.list.member(PrimChecks.validator.checkArg('MEMBER?', 8191, world.observer.getGlobal("visualize-variation")), ["flagella and color", "as color only"])) {
          SelfManager.self().setVariable("color", PrimChecks.list.item(PrimChecks.math.minus(PrimChecks.validator.checkArg('-', 1, SelfManager.self().getVariable("variation")), 1), [115, 105, 55, 35, 25, 15]));
        }
        else {
          SelfManager.self().setVariable("color", world.observer.getGlobal("bacteria-default-color"));
        }
