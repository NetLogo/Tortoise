    ...
    return workspace.rng.withClone(function() {
      return plotManager.withTemporaryContext('Population', 'recyclers')(function() {
        try {
          ...
          var letVars = { };
          plotManager.plotValue(PrimChecks.agentset.count_unchecked(world.turtleManager.turtlesOfBreed("RECYCLERS")));
        } catch (e) {
    ...
    return workspace.rng.withClone(function() {
      return plotManager.withTemporaryContext('Population', 'wastefuls')(function() {
        try {
          var reporterContext = false;
          var letVars = { };
          plotManager.plotValue(PrimChecks.agentset.count_unchecked(world.turtleManager.turtlesOfBreed("WASTEFULS")));
        } catch (e) {
          return Errors.stopInCommandCheck(e)
        };
      });
    ...
    return workspace.rng.withClone(function() {
      return plotManager.withTemporaryContext('Land Use', 'new')(function() {
        try {
          var reporterContext = false;
          var letVars = { };
          plotManager.plotValue(PrimChecks.math.mult_unchecked(PrimChecks.math.div_unchecked(PrimChecks.agentset.countWith(world.patches(), function() { return Prims.equality(SelfManager.self().getPatchVariable("resource-type"), "new"); }), PrimChecks.agentset.count_unchecked(world.patches())), 100));
        } catch (e) {
          return Errors.stopInCommandCheck(e)
        };
      });
    ...
    return workspace.rng.withClone(function() {
      return plotManager.withTemporaryContext('Land Use', 'recycled')(function() {
        try {
          var reporterContext = false;
          var letVars = { };
          plotManager.plotValue(PrimChecks.math.mult_unchecked(PrimChecks.math.div_unchecked(PrimChecks.agentset.countWith(world.patches(), function() { return Prims.equality(SelfManager.self().getPatchVariable("resource-type"), "recycled"); }), PrimChecks.agentset.count_unchecked(world.patches())), 100));
        } catch (e) {
          return Errors.stopInCommandCheck(e)
        };
      });
    ...
    return workspace.rng.withClone(function() {
      return plotManager.withTemporaryContext('Land Use', 'waste')(function() {
        try {
          var reporterContext = false;
          var letVars = { };
          plotManager.plotValue(PrimChecks.math.mult_unchecked(PrimChecks.math.div_unchecked(PrimChecks.agentset.countWith(world.patches(), function() { return Prims.equality(SelfManager.self().getPatchVariable("resource-type"), "waste"); }), PrimChecks.agentset.count_unchecked(world.patches())), 100));
        } catch (e) {
          return Errors.stopInCommandCheck(e)
        };
      });
...
var procedures = (function() {
  ...
  temp = (function() {
    try {
      ...
      Errors.askNobodyCheck(world.turtles()).ask(function() {
        ...
        SelfManager.self().setVariable("energy", PrimChecks.math.div(world.observer.getGlobal("max-stored-energy"), 2));
      }, true);
  ...
  temp = (function() {
    try {
      var reporterContext = false;
      var letVars = { };
      let targetPatch = PrimChecks.list.oneOf_unchecked(SelfManager.self().getNeighbors()); letVars['targetPatch'] = targetPatch;
      if (world.observer.getGlobal("agents-seek-resources?")) {
        let candidateMoves = PrimChecks.agentset.with_unchecked(SelfManager.self().getNeighbors(), function() { return Prims.equality(SelfManager.self().getPatchVariable("resource-type"), "new"); }); letVars['candidateMoves'] = candidateMoves;
        if (PrimChecks.agentset.any(candidateMoves)) {
          targetPatch = PrimChecks.list.oneOf(candidateMoves); letVars['targetPatch'] = targetPatch;
        }
        else {
          candidateMoves = PrimChecks.agentset.with_unchecked(SelfManager.self().getNeighbors(), function() { return Prims.equality(SelfManager.self().getPatchVariable("resource-type"), "recycled"); }); letVars['candidateMoves'] = candidateMoves;
          if (PrimChecks.agentset.any(candidateMoves)) {
            targetPatch = PrimChecks.list.oneOf(candidateMoves); letVars['targetPatch'] = targetPatch;
          }
        }
      ...
      SelfManager.self().setVariable("energy", PrimChecks.math.minus(SelfManager.self().getVariable("energy"), 1));
    } catch (e) {
      return Errors.stopInCommandCheck(e)
    }
  });
  ...
  temp = (function() {
    try {
      ...
      if (Prims.equality(SelfManager.self().getPatchVariable("resource-type"), "new")) {
        if (Prims.lte(SelfManager.self().getVariable("energy"), PrimChecks.math.minus(world.observer.getGlobal("max-stored-energy"), 2))) {
          SelfManager.self().setVariable("energy", PrimChecks.math.plus(SelfManager.self().getVariable("energy"), 2));
        }
      }
      else {
        if (Prims.equality(SelfManager.self().getPatchVariable("resource-type"), "recycled")) {
          if (Prims.lte(SelfManager.self().getVariable("energy"), PrimChecks.math.minus(world.observer.getGlobal("max-stored-energy"), 1))) {
            SelfManager.self().setVariable("energy", PrimChecks.math.plus(SelfManager.self().getVariable("energy"), 1));
          }
        }
        else {
          SelfManager.self().setVariable("energy", PrimChecks.math.minus(SelfManager.self().getVariable("energy"), world.observer.getGlobal("recycling-waste-cost")));
  ...
  temp = (function() {
    try {
      ...
      var letVars = { };
      if (Prims.equality(SelfManager.self().getPatchVariable("resource-type"), "new")) {
        if (Prims.lte(SelfManager.self().getVariable("energy"), PrimChecks.math.minus(world.observer.getGlobal("max-stored-energy"), 4))) {
          SelfManager.self().setVariable("energy", PrimChecks.math.plus(SelfManager.self().getVariable("energy"), 4));
    ...
    try {
      ...
      if (Prims.equality(SelfManager.self().getPatchVariable("resource-type"), "new")) {
        ...
        if (Prims.equality(SelfManager.self().getPatchVariable("resource-type"), "recycled")) {
          ...
          SelfManager.self().setPatchVariable("pcolor", PrimChecks.math.minus_unchecked(45, 1));
        }
      }
    } catch (e) {
      return Errors.stopInCommandCheck(e)
  ...
  temp = (function() {
    try {
      var reporterContext = false;
      var letVars = { };
      Errors.askNobodyCheck(PrimChecks.agentset.with_unchecked(world.patches(), function() { return Prims.equality(SelfManager.self().getPatchVariable("resource-type"), "recycled"); })).ask(function() {
        if (Prims.lt(RandomPrims.randomLong(100), PrimChecks.math.div(world.observer.getGlobal("resource-regeneration"), 10))) {
          SelfManager.self().setPatchVariable("resource-type", "new");
        }
      }, true);
      Errors.askNobodyCheck(PrimChecks.agentset.with_unchecked(world.patches(), function() { return Prims.equality(SelfManager.self().getPatchVariable("resource-type"), "waste"); })).ask(function() {
        if ((Prims.equality(RandomPrims.randomLong(5), 0) && Prims.lt(RandomPrims.randomLong(100), PrimChecks.math.div(world.observer.getGlobal("resource-regeneration"), 10)))) {
          SelfManager.self().setPatchVariable("resource-type", "new");
        }
      }, true);
    } catch (e) {
