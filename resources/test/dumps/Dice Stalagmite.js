    ...
    return workspace.rng.withClone(function() {
      return plotManager.withTemporaryContext('Single Dice', 'default')(function() {
        try {
          ...
          let maxbar = PrimChecks.list.modes(world.observer.getGlobal("single-outcomes")); letVars['maxbar'] = maxbar;
          let maxrange = PrimChecks.list.length_unchecked(PrimChecks.list.filter(Tasks.reporterTask(function(outcome) {
            Errors.procedureArgumentsCheck(1, arguments.length);
            return Prims.equality(outcome, PrimChecks.list.item(0, maxbar));
          }, "[ outcome -> outcome = item 0 maxbar ]"), world.observer.getGlobal("single-outcomes"))); letVars['maxrange'] = maxrange;
          plotManager.setYRange(0, PrimChecks.list.max_unchecked(ListPrims.list(51, maxrange)));
        } catch (e) {
    ...
    return workspace.rng.withClone(function() {
      return plotManager.withTemporaryContext('Pair Sums', 'default')(function() {
        try {
          ...
          let maxbar = PrimChecks.list.modes(world.observer.getGlobal("pair-outcomes")); letVars['maxbar'] = maxbar;
          let maxrange = PrimChecks.list.length_unchecked(PrimChecks.list.filter(Tasks.reporterTask(function(outcome) {
            Errors.procedureArgumentsCheck(1, arguments.length);
            return Prims.equality(outcome, PrimChecks.list.item(0, maxbar));
          }, "[ outcome -> outcome = item 0 maxbar ]"), world.observer.getGlobal("pair-outcomes"))); letVars['maxrange'] = maxrange;
          plotManager.setYRange(0, PrimChecks.list.max_unchecked(ListPrims.list(51, maxrange)));
        } catch (e) {
          return Errors.stopInCommandCheck(e)
        };
...
var procedures = (function() {
  ...
  temp = (function() {
    try {
      ...
      Errors.askNobodyCheck(PrimChecks.agentset.with_unchecked(world.patches(), function() { return Prims.gt(SelfManager.self().getPatchVariable("pxcor"), 4); })).ask(function() {
        SelfManager.self().setPatchVariable("column", PrimChecks.math.floor_unchecked(PrimChecks.math.div_unchecked(PrimChecks.math.minus(SelfManager.self().getPatchVariable("pxcor"), 1), 2)));
      }, true);
      Errors.askNobodyCheck(PrimChecks.agentset.with_unchecked(world.patches(), function() { return Prims.lt(SelfManager.self().getPatchVariable("pxcor"), -4); })).ask(function() {
        SelfManager.self().setPatchVariable("column", PrimChecks.math.plus_unchecked(PrimChecks.math.minus(SelfManager.self().getPatchVariable("pxcor"), world.topology.minPxcor), 1));
      }, true);
      Errors.askNobodyCheck(world.patches()).ask(function() { SelfManager.self().setPatchVariable("pcolor", PrimChecks.math.plus_unchecked(5, 3)); }, true);
      Errors.askNobodyCheck(PrimChecks.agentset.with_unchecked(world.patches(), function() { return !Prims.equality(SelfManager.self().getPatchVariable("column"), 0); })).ask(function() {
        if (Prims.equality(PrimChecks.math.mod(SelfManager.self().getPatchVariable("column"), 2), 0)) {
          ...
          SelfManager.self().setPatchVariable("pcolor", PrimChecks.math.minus_unchecked(35, 1));
        }
      }, true);
  ...
  temp = (function() {
    try {
      var reporterContext = false;
      var letVars = { };
      if (PrimChecks.math.bool('AND', world.observer.getGlobal("stop-at-top?")) && PrimChecks.math.bool('AND', PrimChecks.agentset.any_unchecked(PrimChecks.agentset.turtlesOn(world.observer.getGlobal("top-row"))))) {
        UserDialogPrims.confirm("The top has been reached. Turn STOP-AT-TOP? off to keep going.");
        throw new Exception.StopInterrupt;
      }
      if (PrimChecks.math.not(world.observer.getGlobal("stop-at-top?"))) {
    ...
    try {
      ...
      Errors.askNobodyCheck(world.observer.getGlobal("generators")).ask(function() {
        SelfManager.self().sprout(1, "PAIRED-DICE").ask(function() {
          ...
          SelfManager.self().setVariable("die-value", PrimChecks.math.plus_unchecked(1, RandomPrims.randomLong(6)));
      ...
      Errors.askNobodyCheck(world.turtleManager.turtlesOfBreed("SINGLE-DICE")).ask(function() {
        world.observer.setGlobal("single-outcomes", PrimChecks.list.lput(SelfManager.self().getVariable("die-value"), world.observer.getGlobal("single-outcomes")));
      }, true);
    } catch (e) {
      return Errors.stopInCommandCheck(e)
    }
  ...
  temp = (function() {
    try {
      ...
      if (Prims.gt(howMany, 1)) {
        Errors.askNobodyCheck(PrimChecks.agentset.maxOneOf(world.turtleManager.turtlesOfBreed("SINGLE-DICE"), function() { return SelfManager.self().getPatchVariable("pycor"); })).ask(function() { procedures["MOVE-SINGLE-DIE"](); }, true);
      }
    } catch (e) {
      return Errors.stopInCommandCheck(e)
    }
  ...
  temp = (function() {
    try {
      ...
      if ((Prims.gt(SelfManager.self().getPatchVariable("pycor"), world.topology.minPycor) && PrimChecks.math.not_unchecked(PrimChecks.agentset.any_unchecked(PrimChecks.agentset.breedOn_unchecked("STACKED-DICE", SelfManager.self().patchAhead(1)))))) {
        SelfManager.self()._optimalFdOne();
      }
      else {
  ...
  temp = (function() {
    try {
      ...
      let mode = PrimChecks.list.first_unchecked(PrimChecks.list.modes(world.observer.getGlobal("single-outcomes"))); letVars['mode'] = mode;
      let heightOfTallestColumn = PrimChecks.list.length_unchecked(PrimChecks.list.filter(Tasks.reporterTask(function(outcome) {
        Errors.procedureArgumentsCheck(1, arguments.length);
        return Prims.equality(outcome, mode);
      }, "[ outcome -> outcome = mode ]"), world.observer.getGlobal("single-outcomes"))); letVars['heightOfTallestColumn'] = heightOfTallestColumn;
      let heightOfMyColumn = PrimChecks.list.length_unchecked(PrimChecks.list.filter(Tasks.reporterTask(function(outcome) {
        Errors.procedureArgumentsCheck(1, arguments.length);
        return Prims.equality(outcome, SelfManager.self().getVariable("die-value"));
      }, "[ outcome -> outcome = die-value ]"), world.observer.getGlobal("single-outcomes"))); letVars['heightOfMyColumn'] = heightOfMyColumn;
      if (Prims.gte(PrimChecks.math.minus(heightOfTallestColumn, heightOfMyColumn), PrimChecks.math.minus_unchecked(world.topology.height, 2))) {
  ...
  temp = (function() {
    try {
      ...
      let mode = PrimChecks.list.first_unchecked(PrimChecks.list.modes(world.observer.getGlobal("pair-outcomes"))); letVars['mode'] = mode;
      let heightOfTallestColumn = PrimChecks.list.length_unchecked(PrimChecks.list.filter(Tasks.reporterTask(function(outcome) {
        Errors.procedureArgumentsCheck(1, arguments.length);
        return Prims.equality(outcome, mode);
      }, "[ outcome -> outcome = mode ]"), world.observer.getGlobal("pair-outcomes"))); letVars['heightOfTallestColumn'] = heightOfTallestColumn;
      let heightOfMyColumn = PrimChecks.list.length_unchecked(PrimChecks.list.filter(Tasks.reporterTask(function(outcome) {
        Errors.procedureArgumentsCheck(1, arguments.length);
        return Prims.equality(outcome, SelfManager.self().getVariable("pair-sum"));
      }, "[ outcome -> outcome = pair-sum ]"), world.observer.getGlobal("pair-outcomes"))); letVars['heightOfMyColumn'] = heightOfMyColumn;
      if (Prims.gte(PrimChecks.math.minus(heightOfTallestColumn, heightOfMyColumn), PrimChecks.math.minus_unchecked(world.topology.height, 2))) {
        SelfManager.self().die();
      }
    } catch (e) {
  ...
  temp = (function(candidates) {
    try {
      ...
      while (PrimChecks.agentset.anyWith(candidates, function() {
        return Prims.equality(SelfManager.self().getPatchVariable("pycor"), PrimChecks.math.minus_unchecked(world.topology.maxPycor, 2));
      })) {
