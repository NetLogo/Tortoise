...
var procedures = (function() {
  ...
  temp = (function() {
    try {
      ...
      let mode = PrimChecks.list.first(PrimChecks.list.modes(world.observer.getGlobal("single-outcomes"))); letVars['mode'] = mode;
  ...
  temp = (function() {
    try {
      ...
      let mode = PrimChecks.list.first(PrimChecks.list.modes(world.observer.getGlobal("pair-outcomes"))); letVars['mode'] = mode;
      let heightOfTallestColumn = PrimChecks.list.length(PrimChecks.list.filter(Tasks.reporterTask(function(outcome) {
        Errors.procedureArgumentsCheck(1, arguments.length);
        return Prims.equality(outcome, mode);
  ...
  temp = (function(candidates) {
    try {
      var reporterContext = false;
      var letVars = { };
      while (PrimChecks.agentset.anyWith(candidates, function() { return Prims.equality(SelfManager.self().getPatchVariable("pycor"), (world.topology.maxPycor - 2)); })) {
