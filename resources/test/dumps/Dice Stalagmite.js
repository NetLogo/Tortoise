    ...
    return workspace.rng.withClone(function() {
      return plotManager.withTemporaryContext('Single Dice', 'default')(function() {
        try {
          ...
          let maxbar = PrimChecks.list.modes(PrimChecks.validator.checkArg('MODES', 8, world.observer.getGlobal("single-outcomes"))); letVars['maxbar'] = maxbar;
          let maxrange = PrimChecks.list.length(PrimChecks.list.filter(Tasks.reporterTask(function(outcome) {
          ...
          }, "[ outcome -> outcome = item 0 maxbar ]"), PrimChecks.validator.checkArg('FILTER', 8, world.observer.getGlobal("single-outcomes")))); letVars['maxrange'] = maxrange;
    ...
    return workspace.rng.withClone(function() {
      return plotManager.withTemporaryContext('Pair Sums', 'default')(function() {
        try {
          ...
          let maxbar = PrimChecks.list.modes(PrimChecks.validator.checkArg('MODES', 8, world.observer.getGlobal("pair-outcomes"))); letVars['maxbar'] = maxbar;
          let maxrange = PrimChecks.list.length(PrimChecks.list.filter(Tasks.reporterTask(function(outcome) {
            Errors.procedureArgumentsCheck(1, arguments.length);
            return Prims.equality(outcome, PrimChecks.list.item(0, PrimChecks.validator.checkArg('ITEM', 12, maxbar)));
          }, "[ outcome -> outcome = item 0 maxbar ]"), PrimChecks.validator.checkArg('FILTER', 8, world.observer.getGlobal("pair-outcomes")))); letVars['maxrange'] = maxrange;
          plotManager.setYRange(0, PrimChecks.list.max(ListPrims.list(51, maxrange)));
        } catch (e) {
          return Errors.stopInCommandCheck(e)
        };
...
var procedures = (function() {
  ...
  temp = (function() {
    try {
      ...
      Errors.askNobodyCheck(world.patches()).ask(function() { SelfManager.self().setPatchVariable("pcolor", PrimChecks.math.plus(5, 3)); }, true);
    ...
    try {
      ...
      Errors.askNobodyCheck(world.observer.getGlobal("generators")).ask(function() {
        SelfManager.self().sprout(1, "PAIRED-DICE").ask(function() {
          ...
          SelfManager.self().setVariable("die-value", PrimChecks.math.plus(1, RandomPrims.randomLong(6)));
      ...
      let total = PrimChecks.list.sum(PrimChecks.validator.checkArg('SUM', 8, PrimChecks.agentset.of(world.turtleManager.turtlesOfBreed("PAIRED-DICE"), function() { return SelfManager.self().getVariable("die-value"); }))); letVars['total'] = total;
      ...
      Errors.askNobodyCheck(world.turtleManager.turtlesOfBreed("SINGLE-DICE")).ask(function() {
        world.observer.setGlobal("single-outcomes", PrimChecks.list.lput(PrimChecks.validator.checkArg('LPUT', 8191, SelfManager.self().getVariable("die-value")), PrimChecks.validator.checkArg('LPUT', 8, world.observer.getGlobal("single-outcomes"))));
      }, true);
    } catch (e) {
      return Errors.stopInCommandCheck(e)
    }
  ...
  temp = (function() {
    try {
      ...
      let mode = PrimChecks.list.first(PrimChecks.list.modes(PrimChecks.validator.checkArg('MODES', 8, world.observer.getGlobal("single-outcomes")))); letVars['mode'] = mode;
      let heightOfTallestColumn = PrimChecks.list.length(PrimChecks.list.filter(Tasks.reporterTask(function(outcome) {
        Errors.procedureArgumentsCheck(1, arguments.length);
        return Prims.equality(outcome, mode);
      }, "[ outcome -> outcome = mode ]"), PrimChecks.validator.checkArg('FILTER', 8, world.observer.getGlobal("single-outcomes")))); letVars['heightOfTallestColumn'] = heightOfTallestColumn;
      let heightOfMyColumn = PrimChecks.list.length(PrimChecks.list.filter(Tasks.reporterTask(function(outcome) {
        Errors.procedureArgumentsCheck(1, arguments.length);
        return Prims.equality(outcome, SelfManager.self().getVariable("die-value"));
      }, "[ outcome -> outcome = die-value ]"), PrimChecks.validator.checkArg('FILTER', 8, world.observer.getGlobal("single-outcomes")))); letVars['heightOfMyColumn'] = heightOfMyColumn;
      if (Prims.gte(PrimChecks.math.minus(PrimChecks.validator.checkArg('-', 1, heightOfTallestColumn), PrimChecks.validator.checkArg('-', 1, heightOfMyColumn)), PrimChecks.math.minus(world.topology.height, 2))) {
  ...
  temp = (function() {
    try {
      ...
      let mode = PrimChecks.list.first(PrimChecks.list.modes(PrimChecks.validator.checkArg('MODES', 8, world.observer.getGlobal("pair-outcomes")))); letVars['mode'] = mode;
      let heightOfTallestColumn = PrimChecks.list.length(PrimChecks.list.filter(Tasks.reporterTask(function(outcome) {
        Errors.procedureArgumentsCheck(1, arguments.length);
        return Prims.equality(outcome, mode);
      }, "[ outcome -> outcome = mode ]"), PrimChecks.validator.checkArg('FILTER', 8, world.observer.getGlobal("pair-outcomes")))); letVars['heightOfTallestColumn'] = heightOfTallestColumn;
      let heightOfMyColumn = PrimChecks.list.length(PrimChecks.list.filter(Tasks.reporterTask(function(outcome) {
        Errors.procedureArgumentsCheck(1, arguments.length);
        return Prims.equality(outcome, SelfManager.self().getVariable("pair-sum"));
      }, "[ outcome -> outcome = pair-sum ]"), PrimChecks.validator.checkArg('FILTER', 8, world.observer.getGlobal("pair-outcomes")))); letVars['heightOfMyColumn'] = heightOfMyColumn;
      if (Prims.gte(PrimChecks.math.minus(PrimChecks.validator.checkArg('-', 1, heightOfTallestColumn), PrimChecks.validator.checkArg('-', 1, heightOfMyColumn)), PrimChecks.math.minus(world.topology.height, 2))) {
        SelfManager.self().die();
      }
    } catch (e) {
