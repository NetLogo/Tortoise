...
var procedures = (function() {
  var setup = function() {
    world.clearAll();
    world.patches().ask(function() { SelfPrims.setPatchVariable("pcolor", 55); }, true);
    if (world.observer.getGlobal("grass?")) {
    ...
    Call(procedures.displayLabels);
    world.observer.setGlobal("grass", world.patches().agentFilter(function() { return Prims.equality(SelfPrims.getPatchVariable("pcolor"), 55); }).size());
    world.ticker.reset();
  };
  var go = function() {
    ...
    world.turtleManager.turtlesOfBreed("SHEEP").ask(function() {
      ...
      Call(procedures.reproduceSheep);
    }, true);
    world.turtleManager.turtlesOfBreed("WOLVES").ask(function() {
      Call(procedures.move);
      SelfPrims.setVariable("energy", (SelfPrims.getVariable("energy") - 1));
      Call(procedures.catchSheep);
      Call(procedures.death);
      Call(procedures.reproduceWolves);
    }, true);
    if (world.observer.getGlobal("grass?")) {
      world.patches().ask(function() { Call(procedures.growGrass); }, true);
    }
    world.observer.setGlobal("grass", world.patches().agentFilter(function() { return Prims.equality(SelfPrims.getPatchVariable("pcolor"), 55); }).size());
    world.ticker.tick();
    Call(procedures.displayLabels);
  ...
  var catchSheep = function() {
    ...
    if (!Prims.equality(prey, Nobody)) {
      prey.ask(function() { SelfPrims.die(); }, true);
  ...
  var displayLabels = function() {
    world.turtles().ask(function() { SelfPrims.setVariable("label", ""); }, true);
    if (world.observer.getGlobal("show-energy?")) {
      world.turtleManager.turtlesOfBreed("WOLVES").ask(function() { SelfPrims.setVariable("label", NLMath.round(SelfPrims.getVariable("energy"))); }, true);
      if (world.observer.getGlobal("grass?")) {
        world.turtleManager.turtlesOfBreed("SHEEP").ask(function() { SelfPrims.setVariable("label", NLMath.round(SelfPrims.getVariable("energy"))); }, true);
      }
    }
  };
  return {
