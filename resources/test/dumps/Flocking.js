...
function flock() {
  ...
  if (AgentSet.any(AgentSet.getTurtleVariable(13))) {
    ...
    if (Prims.lt(AgentSet.self().distance(AgentSet.getTurtleVariable(14)), Globals.getGlobal(5))) {
      Call(separate);
    }
    else {
      Call(align);
...
function findFlockmates() {
  AgentSet.setTurtleVariable(13, AgentSet.other(AgentSet.self().inRadius(world.turtles(), Globals.getGlobal(4))));
}
...
function align() {
  Call(turnTowards, Call(averageFlockmateHeading), Globals.getGlobal(1));
}
...
function cohere() {
  Call(turnTowards, Call(averageHeadingTowardsFlockmates), Globals.getGlobal(2));
}
...
function turnTowards(newHeading, maxTurn) {
  Call(turnAtMost, Prims.subtractHeadings(newHeading, AgentSet.getTurtleVariable(2)), maxTurn);
}
function turnAway(newHeading, maxTurn) {
  Call(turnAtMost, Prims.subtractHeadings(AgentSet.getTurtleVariable(2), newHeading), maxTurn);
}
...
Globals.setGlobal(2, 3);
Globals.setGlobal(3, 1.5);
Globals.setGlobal(4, 3);
