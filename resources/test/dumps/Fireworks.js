...
modelConfig.plots = [];
var workspace = tortoise_require('engine/workspace')(modelConfig)([{ name: "rockets", singular: "rocket", varNames: ["terminal-y-vel"] }, { name: "frags", singular: "frag", varNames: ["dim"] }])(["col", "x-vel", "y-vel"], [])('breed [ rockets rocket ] breed [ frags frag ]  globals [   countdown       ; how many ticks to wait before a new salvo of fireworks ]  turtles-own [   col             ; sets color of an explosion particle   x-vel           ; x-velocity   y-vel           ; y-velocity ]  rockets-own [   terminal-y-vel  ; velocity at which rocket will explode ]  frags-own [   dim             ; used for fading particles ]  to setup   clear-all   set-default-shape turtles \"circle\"   reset-ticks end  ; This procedure executes the model. If there are no turtles, ; it will either initiate a new salvo of fireworks by calling ; INIT-ROCKETS or continue to count down if it hasn\'t reached 0. ; It then calls PROJECTILE-MOTION, which launches and explodes ; any currently existing fireworks. to go   if not any? turtles [     ifelse countdown = 0 [       init-rockets       ; use a higher countdown to get a longer pause when trails are drawn       set countdown ifelse-value trails? [ 30 ] [ 10 ]     ] [       ; count down before launching a new salvo       set countdown countdown - 1     ]   ]   ask turtles [ projectile-motion ]   tick end  ; This procedure creates a random number of rockets according to the ; slider FIREWORKS and sets all the initial values for each firework. to init-rockets   clear-drawing   create-rockets (random max-fireworks) + 1 [     setxy random-xcor min-pycor     set x-vel ((random-float (2 * initial-x-vel)) - (initial-x-vel))     set y-vel ((random-float initial-y-vel) + initial-y-vel * 2)     set col one-of base-colors     set color (col + 2)     set size 2     set terminal-y-vel (random-float 4) ; at what speed does the rocket explode?   ] end  ; This function simulates the actual free-fall motion of the turtles. ; If a turtle is a rocket it checks if it has slowed down enough to explode. to projectile-motion ; turtle procedure   set y-vel (y-vel - (gravity / 5))   set heading (atan x-vel y-vel)   let move-amount (sqrt ((x-vel ^ 2) + (y-vel ^ 2)))   if not can-move? move-amount [ die ]   fd (sqrt ((x-vel ^ 2) + (y-vel ^ 2)))    ifelse (breed = rockets) [     if (y-vel < terminal-y-vel) [       explode       die     ]   ] [     fade   ] end  ; This is where the explosion is created. ; EXPLODE calls hatch a number of times indicated by the slider FRAGMENTS. to explode ; turtle procedure   hatch-frags fragments [     set dim 0     rt random 360     set size 1     set x-vel (x-vel * .5 + dx + (random-float 2) - 1)     set y-vel (y-vel * .3 + dy + (random-float 2) - 1)     ifelse trails?       [ pen-down ]       [ pen-up ]   ] end  ; This function changes the color of a frag. ; Each frag fades its color by an amount proportional to FADE-AMOUNT. to fade ; frag procedure   set dim dim - (fade-amount / 10)   set color scale-color col dim -5 .5   if (color < (col - 3.5)) [ die ] end   ; Copyright 1998 Uri Wilensky. ; See Info tab for full copyright and license.')([{"left":244,"top":10,"right":795,"bottom":562,"dimensions":{"minPxcor":-90,"maxPxcor":90,"minPycor":-90,"maxPycor":90,"patchSize":3,"wrappingAllowedInX":true,"wrappingAllowedInY":false},"fontSize":10,"updateMode":"TickBased","showTickCounter":true,"tickCounterLabel":"ticks","frameRate":30,"type":"view","compilation":{"success":true,"messages":[]}}, {"compiledSource":"try {   var reporterContext = false;   var letVars = { };   let _maybestop_33_38 = procedures[\"SETUP\"]();   if (_maybestop_33_38 instanceof Exception.StopInterrupt) { return _maybestop_33_38; } } catch (e) {   return Errors.stopInCommandCheck(e) }","source":"setup","left":9,"top":38,"right":89,"bottom":71,"display":"setup","forever":false,"buttonKind":"Observer","disableUntilTicksStart":false,"type":"button","compilation":{"success":true,"messages":[]}}, {"compiledSource":"try {   var reporterContext = false;   var letVars = { };   let _maybestop_33_35 = procedures[\"GO\"]();   if (_maybestop_33_35 instanceof Exception.StopInterrupt) { return _maybestop_33_35; } } catch (e) {   return Errors.stopInCommandCheck(e) }","source":"go","left":9,"top":88,"right":89,"bottom":121,"display":"go","forever":true,"buttonKind":"Observer","disableUntilTicksStart":true,"type":"button","compilation":{"success":true,"messages":[]}}, {"variable":"trails?","left":4,"top":138,"right":94,"bottom":171,"display":"trails?","on":true,"type":"switch","compilation":{"success":true,"messages":[]}}, {"compiledMin":"1","compiledMax":"40","compiledStep":"1","variable":"max-fireworks","left":112,"top":38,"right":232,"bottom":71,"display":"max-fireworks","min":"1","max":"40","default":20,"step":"1","direction":"horizontal","type":"slider","compilation":{"success":true,"messages":[]}}, {"compiledMin":"5","compiledMax":"50","compiledStep":"1","variable":"fragments","left":112,"top":88,"right":232,"bottom":121,"display":"fragments","min":"5","max":"50","default":30,"step":"1","direction":"horizontal","type":"slider","compilation":{"success":true,"messages":[]}}, {"compiledMin":"0","compiledMax":"5","compiledStep":"0.1","variable":"initial-x-vel","left":112,"top":138,"right":232,"bottom":171,"display":"initial-x-vel","min":"0","max":"5","default":2,"step":"0.1","direction":"horizontal","type":"slider","compilation":{"success":true,"messages":[]}}, {"compiledMin":"0","compiledMax":"5","compiledStep":"0.1","variable":"initial-y-vel","left":112,"top":188,"right":232,"bottom":221,"display":"initial-y-vel","min":"0","max":"5","default":2,"step":"0.1","direction":"horizontal","type":"slider","compilation":{"success":true,"messages":[]}}, {"compiledMin":"0","compiledMax":"3","compiledStep":"0.1","variable":"gravity","left":112,"top":238,"right":232,"bottom":271,"display":"gravity","min":"0","max":"3","default":0.5,"step":"0.1","direction":"horizontal","type":"slider","compilation":{"success":true,"messages":[]}}, {"compiledMin":"0","compiledMax":"10","compiledStep":"0.1","variable":"fade-amount","left":112,"top":288,"right":232,"bottom":321,"display":"fade-amount","min":"0","max":"10","default":1.5,"step":"0.1","direction":"horizontal","type":"slider","compilation":{"success":true,"messages":[]}}])(tortoise_require("extensions/all").porters())(["trails?", "max-fireworks", "fragments", "initial-x-vel", "initial-y-vel", "gravity", "fade-amount", "countdown"], ["trails?", "max-fireworks", "fragments", "initial-x-vel", "initial-y-vel", "gravity", "fade-amount"], [], -90, 90, -90, 90, 3, true, false, turtleShapes, linkShapes, function(){});
...
var procedures = (function() {
  ...
  temp = (function() {
    try {
      ...
      Errors.askNobodyCheck(world.turtles()).ask(function() { procedures["PROJECTILE-MOTION"](); }, true);
      world.ticker.tick();
    } catch (e) {
      return Errors.stopInCommandCheck(e)
    }
