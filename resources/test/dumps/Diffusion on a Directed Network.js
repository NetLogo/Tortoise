  ...
  var pens    = [new PenBundle.Pen('default', plotOps.makePenOps, false, new PenBundle.State(0.0, 1.0, PenBundle.DisplayMode.Bar), function() {}, function() {
    workspace.rng.withAux(function() {
      plotManager.withTemporaryContext('Histogram', 'default')(function() {
        ...
        plotManager.drawHistogramFrom(world.turtles().projectionBy(function() { return SelfPrims.getVariable("val"); }));;
      });
    });
  })];
  var setup   = function() {};
...
var procedures = (function() {
  var setup = function() {
    ...
    world.turtles().ask(function() {
      ...
      var neighborNodes = Prims.turtleSet(SelfPrims.getNeighbors4().projectionBy(function() { return SelfManager.self().turtlesHere(); }));
  ...
  var go = function() {
    world.turtles().ask(function() { SelfPrims.setVariable("new-val", 0); }, true);
    world.turtles().ask(function() {
      ...
      if (recipients.nonEmpty()) {
        ...
        recipients.ask(function() {
          ...
          LinkPrims.inLinkFrom("ACTIVE-LINKS", SelfManager.myself()).ask(function() { SelfPrims.setVariable("current-flow", valIncrement); }, true);
        }, true);
      }
      else {
  ...
  var updateGlobals = function() {
    world.observer.setGlobal("total-val", ListPrims.sum(world.turtles().projectionBy(function() { return SelfPrims.getVariable("val"); })));
    world.observer.setGlobal("max-val", ListPrims.max(world.turtles().projectionBy(function() { return SelfPrims.getVariable("val"); })));
    if (world.linkManager.linksOfBreed("ACTIVE-LINKS").nonEmpty()) {
      world.observer.setGlobal("max-flow", ListPrims.max(world.linkManager.linksOfBreed("ACTIVE-LINKS").projectionBy(function() { return SelfPrims.getVariable("current-flow"); })));
      world.observer.setGlobal("mean-flow", ListPrims.mean(world.linkManager.linksOfBreed("ACTIVE-LINKS").projectionBy(function() { return SelfPrims.getVariable("current-flow"); })));
    }
  };
