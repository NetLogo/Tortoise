    ...
    return workspace.rng.withClone(function() {
      return plotManager.withTemporaryContext('Histogram', 'default')(function() {
        try {
          ...
          var letVars = { };
          plotManager.setXRange(0, PrimChecks.math.ceil((world.observer.getGlobal("max-val") + 0.5)));
          plotManager.setHistogramBarCount(PrimChecks.math.ceil(PrimChecks.math.sqrt(PrimChecks.agentset.count_unchecked(world.turtles()))));
...
var procedures = (function() {
  ...
  temp = (function() {
    try {
      ...
      Errors.askNobodyCheck(world.turtles()).ask(function() {
        SelfManager.self().setXY(PrimChecks.math.div((SelfManager.self().getVariable("xcor") * (world.topology.maxPxcor - 1)), (PrimChecks.math.div(world.observer.getGlobal("grid-size"), 2) - 0.5)), PrimChecks.math.div((SelfManager.self().getVariable("ycor") * (world.topology.maxPycor - 1)), (PrimChecks.math.div(world.observer.getGlobal("grid-size"), 2) - 0.5)));
      }, true);
    ...
    try {
      ...
      Errors.askNobodyCheck(world.turtles()).ask(function() {
        ...
        if (PrimChecks.agentset.any(recipients)) {
          let valToKeep = (SelfManager.self().getVariable("val") * (1 - PrimChecks.math.div(world.observer.getGlobal("diffusion-rate"), 100))); letVars['valToKeep'] = valToKeep;
          SelfManager.self().setVariable("new-val", (SelfManager.self().getVariable("new-val") + valToKeep));
          let valIncrement = PrimChecks.math.div((SelfManager.self().getVariable("val") - valToKeep), PrimChecks.agentset.count(recipients)); letVars['valIncrement'] = valIncrement;
  ...
  temp = (function() {
    try {
      var reporterContext = false;
      var letVars = { };
      SelfManager.self().setVariable("size", (0.1 + (5 * PrimChecks.math.sqrt(PrimChecks.math.div(SelfManager.self().getVariable("val"), world.observer.getGlobal("total-val"))))));
    } catch (e) {
      return Errors.stopInCommandCheck(e)
    }
  });
  ...
  temp = (function() {
    try {
      var reporterContext = false;
      var letVars = { };
      SelfManager.self().setVariable("color", ColorModel.scaleColor(5, PrimChecks.math.div(SelfManager.self().getVariable("current-flow"), ((2 * world.observer.getGlobal("mean-flow")) + 0.00001)), -0.4, 1));
    } catch (e) {
      return Errors.stopInCommandCheck(e)
    }
  });
