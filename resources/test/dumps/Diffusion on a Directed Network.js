    ...
    return workspace.rng.withClone(function() {
      return plotManager.withTemporaryContext('Histogram', 'default')(function() {
        try {
          ...
          var letVars = { };
          plotManager.setXRange(0, PrimChecks.math.ceil_unchecked(PrimChecks.math.plus(world.observer.getGlobal("max-val"), 0.5)));
          plotManager.setHistogramBarCount(PrimChecks.math.ceil_unchecked(PrimChecks.math.sqrt_unchecked(PrimChecks.agentset.count_unchecked(world.turtles()))));
          plotManager.drawHistogramFrom(PrimChecks.agentset.of_unchecked(world.turtles(), function() { return SelfManager.self().getVariable("val"); }));
        } catch (e) {
...
var procedures = (function() {
  ...
  temp = (function() {
    try {
      ...
      Errors.askNobodyCheck(PrimChecks.agentset.with_unchecked(world.patches(), function() {
        return (Prims.lt(PrimChecks.math.abs(SelfManager.self().getPatchVariable("pxcor")), PrimChecks.math.div(world.observer.getGlobal("grid-size"), 2)) && Prims.lt(PrimChecks.math.abs(SelfManager.self().getPatchVariable("pycor")), PrimChecks.math.div(world.observer.getGlobal("grid-size"), 2)));
      })).ask(function() {
      ...
      Errors.askNobodyCheck(world.turtles()).ask(function() {
        ...
        let neighborNodes = PrimChecks.agentset.turtleSet(PrimChecks.agentset.of_unchecked(SelfManager.self().getNeighbors4(), function() { return SelfManager.self().turtlesHere(); })); letVars['neighborNodes'] = neighborNodes;
        LinkPrims.createLinksTo(neighborNodes, "ACTIVE-LINKS").ask(function() {
          SelfManager.self().setVariable("current-flow", 0);
          if (Prims.gt(PrimChecks.math.randomFloat_unchecked(100), world.observer.getGlobal("link-chance"))) {
      ...
      Errors.askNobodyCheck(world.turtles()).ask(function() {
        SelfManager.self().setXY(PrimChecks.math.div_unchecked(PrimChecks.math.mult(SelfManager.self().getVariable("xcor"), PrimChecks.math.minus_unchecked(world.topology.maxPxcor, 1)), PrimChecks.math.minus_unchecked(PrimChecks.math.div(world.observer.getGlobal("grid-size"), 2), 0.5)), PrimChecks.math.div_unchecked(PrimChecks.math.mult(SelfManager.self().getVariable("ycor"), PrimChecks.math.minus_unchecked(world.topology.maxPycor, 1)), PrimChecks.math.minus_unchecked(PrimChecks.math.div(world.observer.getGlobal("grid-size"), 2), 0.5)));
      }, true);
  ...
  temp = (function() {
    try {
      ...
      Errors.askNobodyCheck(world.turtles()).ask(function() {
        ...
        if (PrimChecks.agentset.any(recipients)) {
          let valToKeep = PrimChecks.math.mult(SelfManager.self().getVariable("val"), PrimChecks.math.minus_unchecked(1, PrimChecks.math.div(world.observer.getGlobal("diffusion-rate"), 100))); letVars['valToKeep'] = valToKeep;
          SelfManager.self().setVariable("new-val", PrimChecks.math.plus(SelfManager.self().getVariable("new-val"), valToKeep));
          let valIncrement = PrimChecks.math.div_unchecked(PrimChecks.math.minus(SelfManager.self().getVariable("val"), valToKeep), PrimChecks.agentset.count(recipients)); letVars['valIncrement'] = valIncrement;
          Errors.askNobodyCheck(recipients).ask(function() {
            SelfManager.self().setVariable("new-val", PrimChecks.math.plus(SelfManager.self().getVariable("new-val"), valIncrement));
          ...
          SelfManager.self().setVariable("new-val", PrimChecks.math.plus(SelfManager.self().getVariable("new-val"), SelfManager.self().getVariable("val")));
        }
      }, true);
  ...
  temp = (function() {
    try {
      var reporterContext = false;
      var letVars = { };
      if (PrimChecks.agentset.any_unchecked(world.linkManager.linksOfBreed("ACTIVE-LINKS"))) {
        Errors.askNobodyCheck(PrimChecks.list.oneOf_unchecked(world.linkManager.linksOfBreed("ACTIVE-LINKS"))).ask(function() {
          SelfManager.self().setVariable("breed", world.linkManager.linksOfBreed("INACTIVE-LINKS"));
          SelfManager.self().setVariable('hidden?', true)
        }, true);
        Errors.askNobodyCheck(PrimChecks.list.oneOf_unchecked(world.linkManager.linksOfBreed("INACTIVE-LINKS"))).ask(function() {
  ...
  temp = (function() {
    try {
      var reporterContext = false;
      var letVars = { };
      world.observer.setGlobal("total-val", PrimChecks.list.sum(PrimChecks.agentset.of_unchecked(world.turtles(), function() { return SelfManager.self().getVariable("val"); })));
      world.observer.setGlobal("max-val", PrimChecks.list.max(PrimChecks.agentset.of_unchecked(world.turtles(), function() { return SelfManager.self().getVariable("val"); })));
      if (PrimChecks.agentset.any_unchecked(world.linkManager.linksOfBreed("ACTIVE-LINKS"))) {
        world.observer.setGlobal("max-flow", PrimChecks.list.max(PrimChecks.agentset.of_unchecked(world.linkManager.linksOfBreed("ACTIVE-LINKS"), function() { return SelfManager.self().getVariable("current-flow"); })));
        world.observer.setGlobal("mean-flow", PrimChecks.list.mean(PrimChecks.agentset.of_unchecked(world.linkManager.linksOfBreed("ACTIVE-LINKS"), function() { return SelfManager.self().getVariable("current-flow"); })));
      }
    } catch (e) {
      return Errors.stopInCommandCheck(e)
    }
  ...
  temp = (function() {
    try {
      var reporterContext = false;
      var letVars = { };
      SelfManager.self().setVariable("size", PrimChecks.math.plus_unchecked(0.1, PrimChecks.math.mult_unchecked(5, PrimChecks.math.sqrt_unchecked(PrimChecks.math.div(SelfManager.self().getVariable("val"), world.observer.getGlobal("total-val"))))));
    } catch (e) {
      return Errors.stopInCommandCheck(e)
    }
  });
  ...
  temp = (function() {
    try {
      var reporterContext = false;
      var letVars = { };
      SelfManager.self().setVariable("color", ColorModel.scaleColor(5, PrimChecks.math.div(SelfManager.self().getVariable("current-flow"), PrimChecks.math.plus_unchecked(PrimChecks.math.mult(2, world.observer.getGlobal("mean-flow")), 0.00001)), -0.4, 1));
    } catch (e) {
      return Errors.stopInCommandCheck(e)
    }
  });
