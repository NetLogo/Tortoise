...
function setup() {
  world.clearAll();
  world.observer.setGlobal('clock', 0);
  world.observer.setGlobal('game-started?', false);
  world.observer.setGlobal('game-over?', false);
  BreedManager.setDefaultShape(world.turtleManager.turtlesOfBreed("GRASS-SQUARES").getBreedName(), "grass patch")
  BreedManager.setDefaultShape(world.turtleManager.turtlesOfBreed("MINES").getBreedName(), "bomb")
  BreedManager.setDefaultShape(world.turtleManager.turtlesOfBreed("MARKERS").getBreedName(), "bomb")
  world.patches().ask(function() {
    SelfPrims.sprout(1, 'GRASS-SQUARES').ask(function() {
      SelfPrims.setVariable('color', 55);
    }, true);
    SelfPrims.setPatchVariable('pcolor', 5);
  }, true);
  ListPrims.nOf(world.observer.getGlobal('mine-count'), world.patches()).ask(function() {
    SelfPrims.sprout(1, 'MINES').ask(function() {
...
function go() {
  if (world.observer.getGlobal('game-over?')) {
    world.turtleManager.turtlesOfBreed("MARKERS").agentFilter(function() {
      return SelfManager.self().breedHere("MINES").nonEmpty();
    }).ask(function() {
      SelfPrims.die();
    }, true);
    world.turtleManager.turtlesOfBreed("MARKERS").ask(function() {
      SelfPrims.setVariable('color', (5 - 2));
    }, true);
    world.turtleManager.turtlesOfBreed("MINES").ask(function() {
      SelfManager.self().hideTurtle(false);;
    }, true);
    world.observer.setGlobal('game-over?', true);
    world.patches().ask(function() {
      SelfPrims.setPatchVariable('pcolor', 15);
    }, true);
    throw new Exception.StopInterrupt;
  }
  if (!world.observer.getGlobal('game-started?')) {
    workspace.timer.reset();
    world.observer.setGlobal('game-started?', true);
  }
  world.observer.setGlobal('clock', workspace.timer.elapsed());
  if (world.turtleManager.turtlesOfBreed("GRASS-SQUARES").agentAll(function() {
    return SelfManager.self().breedHere("MINES").nonEmpty();
  })) {
    world.turtleManager.turtlesOfBreed("MINES").ask(function() {
      SelfManager.self().hideTurtle(false);;
  ...
  if (MousePrims.isDown()) {
    world.getPatchAt(StrictMath.round(MousePrims.getX()), StrictMath.round(MousePrims.getY())).ask(function() {
      if (SelfManager.self().breedHere("MINES").nonEmpty()) {
        world.observer.setGlobal('game-over?', true);
      }
      else {
        Call(_clear_);
...
function _clear_() {
  SelfManager.self().breedHere("GRASS-SQUARES").ask(function() {
    SelfPrims.die();
  }, true);
  SelfManager.self().breedHere("MARKERS").ask(function() {
    SelfPrims.die();
  }, true);
  var total = SelfPrims.getNeighbors().agentFilter(function() {
    return SelfManager.self().breedHere("MINES").nonEmpty();
  }).size();
  if (Prims.gt(total, 0)) {
    SelfPrims.setPatchVariable('plabel', total);
  }
  else {
    SelfPrims.getNeighbors().agentFilter(function() {
      return SelfManager.self().breedHere("GRASS-SQUARES").nonEmpty();
    }).ask(function() {
      Call(_clear_);
    }, true);
  }
}
function mark_slash_unmark() {
  if (!MousePrims.isInside()) {
    throw new Exception.StopInterrupt;
  }
  world.getPatchAt(StrictMath.round(MousePrims.getX()), StrictMath.round(MousePrims.getY())).ask(function() {
    if (SelfManager.self().breedHere("GRASS-SQUARES").nonEmpty()) {
      if (SelfManager.self().breedHere("MARKERS").nonEmpty()) {
        SelfManager.self().breedHere("MARKERS").ask(function() {
          SelfPrims.die();
        }, true);
      }
      else {
        SelfPrims.sprout(1, 'MARKERS').ask(function() {
          SelfPrims.setVariable('color', 0);
        }, true);
      }
    }
...
world.observer.setGlobal('mine-count', 50);
