...
var procedures = (function() {
  ...
  temp = (function() {
    try {
      ...
      if (PrimChecks.agentset.anyWith(world.turtleManager.turtlesOfBreed("PARTICLES"), function() { return Prims.gt(SelfManager.self().getVariable("speed"), 0); })) {
        world.observer.setGlobal("tick-delta", PrimChecks.list.min_unchecked(ListPrims.list(PrimChecks.math.div_unchecked(1, PrimChecks.math.ceil_unchecked(PrimChecks.list.max(PrimChecks.agentset.of_unchecked(world.turtleManager.turtlesOfBreed("PARTICLES"), function() { return SelfManager.self().getVariable("speed"); })))), world.observer.getGlobal("max-tick-delta"))));
      }
      else {
  ...
  temp = (function() {
    try {
      var reporterContext = false;
      var letVars = { };
      if (!Prims.equality(SelfManager.self().patchAhead(PrimChecks.math.mult(SelfManager.self().getVariable("speed"), world.observer.getGlobal("tick-delta"))), SelfManager.self().getPatchHere())) {
  ...
  temp = (function() {
    try {
      var reporterContext = false;
      var letVars = { };
      SelfManager.self().setXY(PrimChecks.math.plus_unchecked(PrimChecks.math.plus_unchecked(1, world.topology.minPxcor), PrimChecks.math.randomFloat_unchecked(PrimChecks.math.minus_unchecked(PrimChecks.math.mult_unchecked(2, world.topology.maxPxcor), 2))), PrimChecks.math.plus_unchecked(PrimChecks.math.plus_unchecked(1, world.topology.minPycor), PrimChecks.math.randomFloat_unchecked(PrimChecks.math.minus_unchecked(PrimChecks.math.mult_unchecked(2, world.topology.maxPycor), 2))));
    } catch (e) {
      return Errors.stopInCommandCheck(e)
    }
  });
