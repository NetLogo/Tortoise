    ...
    return workspace.rng.withClone(function() {
      return plotManager.withTemporaryContext('Energy Histogram', undefined)(function() {
        try {
          ...
          plotManager.setYRange(0, PrimChecks.math.ceil(PrimChecks.math.div(world.observer.getGlobal("number-of-particles"), 6)));
        } catch (e) {
          return Errors.stopInCommandCheck(e)
        };
      });
...
var procedures = (function() {
  ...
  temp = (function() {
    try {
      ...
      if (PrimChecks.agentset.anyWith(world.turtleManager.turtlesOfBreed("PARTICLES"), function() { return Prims.gt(SelfManager.self().getVariable("speed"), 0); })) {
        world.observer.setGlobal("tick-delta", PrimChecks.list.min(ListPrims.list(PrimChecks.math.div(1, PrimChecks.math.ceil(PrimChecks.list.max(PrimChecks.agentset.of_unchecked(world.turtleManager.turtlesOfBreed("PARTICLES"), function() { return SelfManager.self().getVariable("speed"); })))), world.observer.getGlobal("max-tick-delta"))));
      }
      else {
  ...
  temp = (function() {
    try {
      var reporterContext = false;
      var letVars = { };
      SelfManager.self().setXY(((1 + world.topology.minPxcor) + PrimChecks.math.randomFloat(((2 * world.topology.maxPxcor) - 2))), ((1 + world.topology.minPycor) + PrimChecks.math.randomFloat(((2 * world.topology.maxPycor) - 2))));
    } catch (e) {
      return Errors.stopInCommandCheck(e)
    }
  });
