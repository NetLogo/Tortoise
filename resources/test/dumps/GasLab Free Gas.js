...
var procedures = (function() {
  ...
  temp = (function() {
    try {
      var reporterContext = false;
      var letVars = { };
      if (!Prims.equality(SelfManager.self().patchAhead(PrimChecks.math.mult(PrimChecks.validator.checkArg('*', 1, SelfManager.self().getVariable("speed")), PrimChecks.validator.checkArg('*', 1, world.observer.getGlobal("tick-delta")))), SelfManager.self().getPatchHere())) {
  ...
  temp = (function() {
    try {
      var reporterContext = false;
      var letVars = { };
      SelfManager.self().setXY(PrimChecks.math.plus(PrimChecks.math.plus(1, world.topology.minPxcor), PrimChecks.math.randomFloat(PrimChecks.math.minus(PrimChecks.math.mult(2, world.topology.maxPxcor), 2))), PrimChecks.math.plus(PrimChecks.math.plus(1, world.topology.minPycor), PrimChecks.math.randomFloat(PrimChecks.math.minus(PrimChecks.math.mult(2, world.topology.maxPycor), 2))));
    } catch (e) {
      return Errors.stopInCommandCheck(e)
    }
  });
