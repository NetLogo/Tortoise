...
var procedures = (function() {
  ...
  temp = (function() {
    try {
      var reporterContext = false;
      var letVars = { };
      let anyVotesChanged_p = false; letVars['anyVotesChanged_p'] = anyVotesChanged_p;
      world.patches().ask(function() { SelfManager.self().setPatchVariable("total", SelfManager.self()._optimalNSum("vote")); }, true);
      world.patches().ask(function() {
        let previousVote = SelfManager.self().getPatchVariable("vote"); letVars['previousVote'] = previousVote;
        ...
        if (!Prims.equality(SelfManager.self().getPatchVariable("vote"), previousVote)) {
          anyVotesChanged_p = true; letVars['anyVotesChanged_p'] = anyVotesChanged_p;
        }
        procedures["RECOLOR-PATCH"]();
      }, true);
