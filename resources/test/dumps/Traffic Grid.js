...
var procedures = (function() {
  ...
  var makeCurrent = function(light) {
    ...
    world.observer.setGlobal("current-phase", world.observer.getGlobal("current-light").projectionBy(function() { return SelfPrims.getPatchVariable("my-phase"); }));
    world.observer.setGlobal("current-auto?", world.observer.getGlobal("current-light").projectionBy(function() { return SelfPrims.getPatchVariable("auto?"); }));
  };
  ...
  var unlabelCurrent = function() {
    world.observer.getGlobal("current-light").ask(function() {
      SelfManager.self().patchAt(-1, 1).ask(function() { SelfPrims.setPatchVariable("plabel", ""); }, true);
    }, true);
  };
  ...
  var setSignalColors = function() {
    if (world.observer.getGlobal("power?")) {
      if (SelfPrims.getPatchVariable("green-light-up?")) {
        SelfManager.self().patchAt(-1, 0).ask(function() { SelfPrims.setPatchVariable("pcolor", 15); }, true);
        SelfManager.self().patchAt(0, 1).ask(function() { SelfPrims.setPatchVariable("pcolor", 55); }, true);
      }
      else {
        SelfManager.self().patchAt(-1, 0).ask(function() { SelfPrims.setPatchVariable("pcolor", 55); }, true);
        SelfManager.self().patchAt(0, 1).ask(function() { SelfPrims.setPatchVariable("pcolor", 15); }, true);
      }
    }
    else {
      SelfManager.self().patchAt(-1, 0).ask(function() { SelfPrims.setPatchVariable("pcolor", 9.9); }, true);
      SelfManager.self().patchAt(0, 1).ask(function() { SelfPrims.setPatchVariable("pcolor", 9.9); }, true);
    }
  };
  ...
  var setSpeed = function(deltaX, deltaY) {
    ...
    if (turtlesAhead.nonEmpty()) {
      if (turtlesAhead.agentFilter(function() {
        return !Prims.equality(SelfPrims.getVariable("up-car?"), SelfManager.myself().projectionBy(function() { return SelfPrims.getVariable("up-car?"); }));
      }).nonEmpty()) {
        SelfPrims.setVariable("speed", 0);
      }
      else {
