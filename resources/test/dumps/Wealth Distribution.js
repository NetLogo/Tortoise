...
function setupPatches() {
  AgentSet.ask(world.patches(), true, function() {
    ...
    if (Prims.lte(Prims.randomFloat(100), Globals.getGlobal(4))) {
      AgentSet.setPatchVariable(6, Globals.getGlobal(8));
  ...
  Prims.repeat(5, function () {
    ...
    world.topology().diffuse(5, 0.25);
  });
  Prims.repeat(10, function () {
    world.topology().diffuse(5, 0.25);
  });
  AgentSet.ask(world.patches(), true, function() {
    AgentSet.setPatchVariable(5, StrictMath.floor(AgentSet.getPatchVariable(5)));
    AgentSet.setPatchVariable(6, AgentSet.getPatchVariable(5));
    Call(recolorPatch);
  });
}
...
function setupTurtles() {
  Breeds.setDefaultShape(world.turtles(), "person");
  AgentSet.ask(world.createTurtles(Globals.getGlobal(3), ""), true, function() {
    ...
    AgentSet.setTurtleVariable(13, Prims.random(AgentSet.getTurtleVariable(15)));
  });
  Call(recolorTurtles);
}
function setInitialTurtleVars() {
  ...
  AgentSet.setTurtleVariable(15, (Globals.getGlobal(7) + Prims.random(((Globals.getGlobal(5) - Globals.getGlobal(7)) + 1))));
  AgentSet.setTurtleVariable(16, (1 + Prims.random(Globals.getGlobal(2))));
  AgentSet.setTurtleVariable(14, (AgentSet.getTurtleVariable(16) + Prims.random(50)));
  AgentSet.setTurtleVariable(17, (1 + Prims.random(Globals.getGlobal(0))));
}
function recolorTurtles() {
  ...
  AgentSet.ask(world.turtles(), true, function() {
    if (Prims.lte(AgentSet.getTurtleVariable(14), (maxWealth / 3))) {
      ...
      if (Prims.lte(AgentSet.getTurtleVariable(14), ((maxWealth * 2) / 3))) {
...
function turnTowardsGrain() {
  ...
  AgentSet.setTurtleVariable(2, bestDirection);
}
function grainAhead() {
  var total = 0;
  var howFar = 1;
...
function growGrain() {
  if (Prims.lt(AgentSet.getPatchVariable(5), AgentSet.getPatchVariable(6))) {
    AgentSet.setPatchVariable(5, (AgentSet.getPatchVariable(5) + Globals.getGlobal(6)));
...
function harvest() {
  AgentSet.ask(world.turtles(), true, function() {
    AgentSet.setTurtleVariable(14, StrictMath.floor((AgentSet.getTurtleVariable(14) + (AgentSet.getPatchVariable(5) / AgentSet.count(AgentSet.self().turtlesHere())))));
  });
  AgentSet.ask(world.turtles(), true, function() {
...
function moveEatAgeDie() {
  Prims.fd(1);
  AgentSet.setTurtleVariable(14, (AgentSet.getTurtleVariable(14) - AgentSet.getTurtleVariable(16)));
  AgentSet.setTurtleVariable(13, (AgentSet.getTurtleVariable(13) + 1));
  if ((Prims.lt(AgentSet.getTurtleVariable(14), 0) || Prims.gte(AgentSet.getTurtleVariable(13), AgentSet.getTurtleVariable(15)))) {
    Call(setInitialTurtleVars);
  }
}
function updateLorenzAndGini() {
  ...
  Prims.repeat(Globals.getGlobal(3), function () {
    ...
    Globals.setGlobal(10, Prims.lput(((wealthSumSoFar / totalWealth) * 100), Globals.getGlobal(10)));
    index = (index + 1);
    Globals.setGlobal(9, ((Globals.getGlobal(9) + (index / Globals.getGlobal(3))) - (wealthSumSoFar / totalWealth)));
  });
}
Globals.setGlobal(0, 5);
Globals.setGlobal(1, 1);
Globals.setGlobal(2, 15);
Globals.setGlobal(3, 250);
Globals.setGlobal(4, 10);
Globals.setGlobal(5, 83);
Globals.setGlobal(6, 4);
