...
var procedures = (function() {
  ...
  temp = (function() {
    try {
      ...
      if ((Prims.gte(PrimChecks.agentset.count(hitHydrogen), 3) && Prims.equality(RandomPrims.randomLong(2), 0))) {
        ...
        let totalInputEnergy = (SelfManager.self().getVariable("energy") + PrimChecks.list.sum(PrimChecks.agentset.of(reactants, function() { return SelfManager.self().getVariable("energy"); }))); letVars['totalInputEnergy'] = totalInputEnergy;
  ...
  temp = (function() {
    try {
      ...
      if ((!SelfManager.self().getPatchVariable("wall?") && PrimChecks.agentset.of(SelfManager.self().patchAt(SelfManager.self().dx(), SelfManager.self().dy()), function() { return !SelfManager.self().getPatchVariable("wall?"); }))) {
        throw new Exception.StopInterrupt;
      }
