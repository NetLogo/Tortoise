...
function createHistogramWidth() {
  AgentSet.ask(world.patches(), true, function() {
    if (((Prims.gte(AgentSet.getPatchVariable(0), ((- world.observer.getGlobal('sample-space')) / 2)) && Prims.lt(AgentSet.getPatchVariable(0), (world.observer.getGlobal('sample-space') / 2))) && Prims.lt(AgentSet.getPatchVariable(1), world.observer.getGlobal('max-y-histogram')))) {
...
function setupColumnCounters() {
  AgentSet.ask(AgentSet.agentFilter(world.patches(), function() {
    ...
    AgentSet.ask(Prims.sprout(1, "COLUMN-COUNTERS"), true, function() {
      ...
      AgentSet.setBreedVariable("MY-COLUMN", StrictMath.floor(((AgentSet.getPatchVariable(0) + (world.observer.getGlobal('sample-space') / 2)) + 1)));
...
function selectRandomValue() {
  AgentSet.ask(Prims.patch(0, (world.observer.getGlobal('max-y-histogram') + 4)), true, function() {
    AgentSet.ask(Prims.sprout(1, "MESSENGERS"), true, function() {
      ...
      AgentSet.setTurtleVariable(6, (1 + Prims.random(world.observer.getGlobal('sample-space'))));
...
function sendMessengerToItsColumn() {
  ...
  AgentSet.ask(it, true, function() {
    Call(createFrame);
    Prims.fd(1);
    if (Prims.equality(AgentSet.getTurtleVariable(4), world.observer.getGlobal('max-y-histogram'))) {
