...
var typeIsArray    = require('integration/typeisarray');
      Breeds.add("COLUMN-COUNTERS", "column-counter");
Breeds.get("COLUMN-COUNTERS").vars =["MY-COLUMN", "MY-COLUMN-PATCHES"];
Breeds.add("FRAMES", "frame");
Breeds.get("FRAMES").vars =[""];
Breeds.add("MESSENGERS", "messenger");
Breeds.get("MESSENGERS").vars =[""];
function setup() {
  world.clearAll();
  Globals.setGlobal(6, (world.minPycor + Globals.getGlobal(3)));
...
function createHistogramWidth() {
  AgentSet.ask(world.patches(), true, function() {
    if (((Prims.gte(AgentSet.getPatchVariable(0), ((- Globals.getGlobal(2)) / 2)) && Prims.lt(AgentSet.getPatchVariable(0), (Globals.getGlobal(2) / 2))) && Prims.lt(AgentSet.getPatchVariable(1), Globals.getGlobal(6)))) {
...
function setupColumnCounters() {
  AgentSet.ask(AgentSet.agentFilter(world.patches(), function() {
    ...
    AgentSet.ask(Prims.sprout(1, "COLUMN-COUNTERS"), true, function() {
      ...
      AgentSet.setBreedVariable("MY-COLUMN", StrictMath.floor(((AgentSet.getPatchVariable(0) + (Globals.getGlobal(2) / 2)) + 1)));
...
function selectRandomValue() {
  AgentSet.ask(Prims.patch(0, (Globals.getGlobal(6) + 4)), true, function() {
    AgentSet.ask(Prims.sprout(1, "MESSENGERS"), true, function() {
      ...
      AgentSet.setTurtleVariable(6, (1 + Prims.random(Globals.getGlobal(2))));
      Globals.setGlobal(5, AgentSet.self());
    });
  });
}
function sendMessengerToItsColumn() {
  var it = AgentSet.oneOf(AgentSet.agentFilter(world.turtlesOfBreed("COLUMN-COUNTERS"), function() {
    return Prims.equality(AgentSet.getBreedVariable("MY-COLUMN"), AgentSet.of(Globals.getGlobal(5), function() {
      return AgentSet.getTurtleVariable(6);
    }));
  }));
  AgentSet.ask(Globals.getGlobal(5), true, function() {
  ...
  AgentSet.ask(it, true, function() {
    Call(createFrame);
    Prims.fd(1);
    if (Prims.equality(AgentSet.getTurtleVariable(4), Globals.getGlobal(6))) {
      Globals.setGlobal(4, true);
    }
  });
}
...
function paint() {
  AgentSet.ask(world.turtlesOfBreed("COLUMN-COUNTERS"), true, function() {
    if (Prims.lte(AgentSet.getBreedVariable("MY-COLUMN"), ((Globals.getGlobal(0) * Globals.getGlobal(2)) / 100))) {
...
function _percent_Full() {
  return Prims.precision(((100 * AgentSet.count(world.turtlesOfBreed("FRAMES"))) / (Globals.getGlobal(3) * Globals.getGlobal(2))), 2);
}
function biggestGap() {
...
Globals.setGlobal(2, 100);
Globals.setGlobal(3, 30);
