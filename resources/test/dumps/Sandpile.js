    ...
    return workspace.rng.withClone(function() {
      return plotManager.withTemporaryContext('Average grain count', 'average')(function() {
        try {
          ...
          var letVars = { };
          plotManager.plotValue(PrimChecks.math.div(world.observer.getGlobal("total"), PrimChecks.agentset.count_unchecked(world.patches())));
        } catch (e) {
    ...
    return workspace.rng.withClone(function() {
      return plotManager.withTemporaryContext('Avalanche sizes', 'default')(function() {
        try {
          var reporterContext = false;
          var letVars = { };
          if ((Prims.equality(PrimChecks.math.mod_unchecked(world.ticker.tickCount(), 100), 0) && PrimChecks.math.not_unchecked(PrimChecks.list.empty(world.observer.getGlobal("sizes"))))) {
            plotManager.resetPen();
            let counts = Tasks.nValues(PrimChecks.math.plus_unchecked(1, PrimChecks.list.max(world.observer.getGlobal("sizes"))), Tasks.reporterTask(function() { return 0; }, "[ 0 ]")); letVars['counts'] = counts;
            var _foreach_138_145 = Tasks.forEach(Tasks.commandTask(function(theSize) {
              Errors.procedureArgumentsCheck(1, arguments.length);
              counts = PrimChecks.list.replaceItem(theSize, counts, PrimChecks.math.plus(1, PrimChecks.list.item(theSize, counts))); letVars['counts'] = counts;
            ...
            var _foreach_253_260 = Tasks.forEach(Tasks.commandTask(function(c) {
              ...
              if ((Prims.gt(s, 0) && Prims.gt(c, 0))) {
                plotManager.plotPoint(PrimChecks.math.log(s, 10), PrimChecks.math.log(c, 10));
              }
              s = PrimChecks.math.plus(s, 1); letVars['s'] = s;
    ...
    return workspace.rng.withClone(function() {
      return plotManager.withTemporaryContext('Avalanche lifetimes', 'default')(function() {
        try {
          var reporterContext = false;
          var letVars = { };
          if ((Prims.equality(PrimChecks.math.mod_unchecked(world.ticker.tickCount(), 100), 0) && PrimChecks.math.not_unchecked(PrimChecks.list.empty(world.observer.getGlobal("lifetimes"))))) {
            plotManager.resetPen();
            let counts = Tasks.nValues(PrimChecks.math.plus_unchecked(1, PrimChecks.list.max(world.observer.getGlobal("lifetimes"))), Tasks.reporterTask(function() { return 0; }, "[ 0 ]")); letVars['counts'] = counts;
            var _foreach_146_153 = Tasks.forEach(Tasks.commandTask(function(lifetime) {
              Errors.procedureArgumentsCheck(1, arguments.length);
              counts = PrimChecks.list.replaceItem(lifetime, counts, PrimChecks.math.plus(1, PrimChecks.list.item(lifetime, counts))); letVars['counts'] = counts;
            ...
            var _foreach_265_272 = Tasks.forEach(Tasks.commandTask(function(c) {
              ...
              if ((Prims.gt(l, 0) && Prims.gt(c, 0))) {
                plotManager.plotPoint(PrimChecks.math.log(l, 10), PrimChecks.math.log(c, 10));
              }
              l = PrimChecks.math.plus(l, 1); letVars['l'] = l;
...
var procedures = (function() {
  ...
  temp = (function(setupTask) {
    try {
      ...
      world.observer.setGlobal("total", PrimChecks.list.sum(PrimChecks.agentset.of_unchecked(world.patches(), function() { return SelfManager.self().getPatchVariable("n"); })));
  ...
  temp = (function() {
    try {
      ...
      if (!Prims.equality(drop, Nobody)) {
        ...
        let avalanchePatches = PrimChecks.list.first(results); letVars['avalanchePatches'] = avalanchePatches;
        let lifetime = PrimChecks.list.last(results); letVars['lifetime'] = lifetime;
        if (PrimChecks.agentset.any(avalanchePatches)) {
          world.observer.setGlobal("sizes", PrimChecks.list.lput(PrimChecks.agentset.count(avalanchePatches), world.observer.getGlobal("sizes")));
          world.observer.setGlobal("lifetimes", PrimChecks.list.lput(lifetime, world.observer.getGlobal("lifetimes")));
        }
  ...
  temp = (function(animate_p) {
    try {
      ...
      var letVars = { };
      let activePatches = PrimChecks.agentset.with_unchecked(world.patches(), function() { return Prims.gt(SelfManager.self().getPatchVariable("n"), 3); }); letVars['activePatches'] = activePatches;
      let iters = 0; letVars['iters'] = iters;
      let avalanchePatches = new PatchSet([], world); letVars['avalanchePatches'] = avalanchePatches;
      while (PrimChecks.agentset.any(activePatches)) {
        let overloadedPatches = PrimChecks.agentset.with(activePatches, function() { return Prims.gt(SelfManager.self().getPatchVariable("n"), 3); }); letVars['overloadedPatches'] = overloadedPatches;
        if (PrimChecks.agentset.any(overloadedPatches)) {
          iters = PrimChecks.math.plus(iters, 1); letVars['iters'] = iters;
        }
  ...
  temp = (function(howMuch) {
    try {
      var reporterContext = false;
      var letVars = { };
      SelfManager.self().setPatchVariable("n", PrimChecks.math.plus(SelfManager.self().getPatchVariable("n"), howMuch));
      world.observer.setGlobal("total", PrimChecks.math.plus(world.observer.getGlobal("total"), howMuch));
    } catch (e) {
      return Errors.stopInCommandCheck(e)
    }
  });
  ...
  temp = (function() {
    try {
      ...
      if (Prims.equality(world.observer.getGlobal("drop-location"), "random")) {
        Errors.reportInContextCheck(reporterContext);
        return PrimChecks.list.oneOf_unchecked(world.patches());
      }
  ...
  temp = (function() {
    try {
      var reporterContext = false;
      var letVars = { };
      SelfManager.self().setPatchVariable("n-stack", PrimChecks.list.fput(SelfManager.self().getPatchVariable("n"), SelfManager.self().getPatchVariable("n-stack")));
    } catch (e) {
      return Errors.stopInCommandCheck(e)
    }
  });
  ...
  temp = (function() {
    try {
      var reporterContext = false;
      var letVars = { };
      procedures["UPDATE-N"](PrimChecks.math.minus(PrimChecks.list.first(SelfManager.self().getPatchVariable("n-stack")), SelfManager.self().getPatchVariable("n")));
      SelfManager.self().setPatchVariable("n-stack", PrimChecks.list.butLast('but-last', SelfManager.self().getPatchVariable("n-stack")));
    } catch (e) {
      return Errors.stopInCommandCheck(e)
    }
  });
