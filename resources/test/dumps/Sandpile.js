    ...
    return workspace.rng.withClone(function() {
      return plotManager.withTemporaryContext('Avalanche sizes', 'default')(function() {
        try {
          var reporterContext = false;
          var letVars = { };
          if ((Prims.equality(PrimChecks.math.mod(world.ticker.tickCount(), 100), 0) && !PrimChecks.list.empty(world.observer.getGlobal("sizes")))) {
            plotManager.resetPen();
            let counts = Tasks.nValues((1 + PrimChecks.list.max(world.observer.getGlobal("sizes"))), Tasks.reporterTask(function() { return 0; }, "[ 0 ]")); letVars['counts'] = counts;
            var _foreach_138_145 = Tasks.forEach(Tasks.commandTask(function(theSize) {
              Errors.procedureArgumentsCheck(1, arguments.length);
              counts = PrimChecks.list.replaceItem(theSize, counts, (1 + PrimChecks.list.item(theSize, counts))); letVars['counts'] = counts;
    ...
    return workspace.rng.withClone(function() {
      return plotManager.withTemporaryContext('Avalanche lifetimes', 'default')(function() {
        try {
          var reporterContext = false;
          var letVars = { };
          if ((Prims.equality(PrimChecks.math.mod(world.ticker.tickCount(), 100), 0) && !PrimChecks.list.empty(world.observer.getGlobal("lifetimes")))) {
            plotManager.resetPen();
            let counts = Tasks.nValues((1 + PrimChecks.list.max(world.observer.getGlobal("lifetimes"))), Tasks.reporterTask(function() { return 0; }, "[ 0 ]")); letVars['counts'] = counts;
            var _foreach_146_153 = Tasks.forEach(Tasks.commandTask(function(lifetime) {
              Errors.procedureArgumentsCheck(1, arguments.length);
              counts = PrimChecks.list.replaceItem(lifetime, counts, (1 + PrimChecks.list.item(lifetime, counts))); letVars['counts'] = counts;
...
var procedures = (function() {
  ...
  temp = (function() {
    try {
      var reporterContext = false;
      var letVars = { };
      procedures["UPDATE-N"]((PrimChecks.list.first(SelfManager.self().getPatchVariable("n-stack")) - SelfManager.self().getPatchVariable("n")));
