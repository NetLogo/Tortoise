...
ProcedurePrims.defineCommand("setup", 1265, 1703, (function(setupHtask) {
  world.clearAll();
  world.observer.setVariable("default-color", 105);
  world.observer.setVariable("fired-color", 15);
  world.observer.setVariable("selected-color", 55);
  world.observer.setVariable("selected-patch", Nobody);
  ...
  world.observer.setVariable("total", PrimChecks.list.sum(1570, 1573, PrimChecks.validator.checkArg('SUM', 1570, 1573, 8, PrimChecks.agentset.of(world.patches(), function() { return PrimChecks.patch.getVariable(1576, 1577, "n"); }))));
  world.observer.setVariable("sizes", []);
  world.observer.setVariable("lifetimes", []);
  world.ticker.reset();
}))
...
ProcedurePrims.defineCommand("go", 2054, 2822, (function() {
  ...
  if (!Prims.equality(drop, Nobody)) {
    ...
    if (PrimChecks.agentset.any(PrimChecks.validator.checkArg('ANY?', 2360, 2364, 112, avalancheHpatches))) {
      world.observer.setVariable("sizes", PrimChecks.list.lput(PrimChecks.agentset.count(PrimChecks.validator.checkArg('COUNT', 2407, 2412, 112, avalancheHpatches)), PrimChecks.validator.checkArg('LPUT', 2401, 2405, 8, world.observer.getGlobal("sizes"))));
      world.observer.setVariable("lifetimes", PrimChecks.list.lput(lifetime, PrimChecks.validator.checkArg('LPUT', 2458, 2462, 8, world.observer.getGlobal("lifetimes"))));
    }
    ...
    world.observer.setVariable("total-on-tick", world.observer.getGlobal("total"));
    world.ticker.tick();
  }
}))
ProcedurePrims.defineCommand("explore", 2830, 3367, (function() {
  if (MousePrims.isInside()) {
    ...
    world.observer.setVariable("selected-patch", p);
    ...
    var R = ProcedurePrims.ask(PrimChecks.validator.checkArg('ASK', 2957, 2960, 1904, world.observer.getGlobal("selected-patch")), function() { var R = ProcedurePrims.callCommand("update-n", 1); if (R === DeathInterrupt) { return R; } }, true); if (R !== undefined) { PrimChecks.procedure.preReturnCheck(R); return R; }
...
ProcedurePrims.defineCommand("update-n", 4951, 5023, (function(howHmuch) {
  ...
  world.observer.setVariable("total", PrimChecks.math.plus(5012, 5013, PrimChecks.validator.checkArg('+', 5012, 5013, 1, world.observer.getGlobal("total")), PrimChecks.validator.checkArg('+', 5012, 5013, 1, howHmuch)));
}))
...
world.observer.setVariable("animate-avalanches?", false);
world.observer.setVariable("drop-location", "random");
world.observer.setVariable("grains-per-patch", 0);