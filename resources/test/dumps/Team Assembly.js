    ...
    return workspace.rng.withClone(function() {
      return plotManager.withTemporaryContext('Link counts', undefined)(function() {
        try {
          ...
          total = PrimChecks.math.plus(total, PrimChecks.agentset.countWith(world.links(), function() { return Prims.equality(SelfManager.self().getVariable("color"), 15); })); letVars['total'] = total;
          plotManager.lowerPen();
          plotManager.plotPoint(world.ticker.tickCount(), total);
          plotManager.setCurrentPen("incumbent-incumbent");
          plotManager.raisePen();
          plotManager.plotPoint(world.ticker.tickCount(), total);
          total = PrimChecks.math.plus(total, PrimChecks.agentset.countWith(world.links(), function() { return Prims.equality(SelfManager.self().getVariable("color"), 45); })); letVars['total'] = total;
          plotManager.lowerPen();
          plotManager.plotPoint(world.ticker.tickCount(), total);
          plotManager.setCurrentPen("newcomer-incumbent");
          plotManager.raisePen();
          plotManager.plotPoint(world.ticker.tickCount(), total);
          total = PrimChecks.math.plus(total, PrimChecks.agentset.countWith(world.links(), function() { return Prims.equality(SelfManager.self().getVariable("color"), 75); })); letVars['total'] = total;
          plotManager.lowerPen();
          plotManager.plotPoint(world.ticker.tickCount(), total);
          plotManager.setCurrentPen("newcomer-newcomer");
          plotManager.raisePen();
          plotManager.plotPoint(world.ticker.tickCount(), total);
          total = PrimChecks.math.plus(total, PrimChecks.agentset.countWith(world.links(), function() { return Prims.equality(SelfManager.self().getVariable("color"), 105); })); letVars['total'] = total;
          plotManager.lowerPen();
          plotManager.plotPoint(world.ticker.tickCount(), total);
        } catch (e) {
...
var procedures = (function() {
  ...
  temp = (function() {
    try {
      ...
      world.turtleManager.createTurtles(1, "").ask(function() {
        SelfManager.self().setVariable("color", PrimChecks.math.plus_unchecked(105, 1));
  ...
  temp = (function() {
    try {
      ...
      Errors.askNobodyCheck(world.turtles()).ask(function() {
        SelfManager.self().setVariable("heading", PrimChecks.math.mult(PrimChecks.math.div(360, world.observer.getGlobal("team-size")), SelfManager.self().getVariable("who")));
  ...
  temp = (function() {
    try {
      ...
      Errors.askNobodyCheck(world.turtles()).ask(function() {
        SelfManager.self().setVariable("incumbent?", true);
        SelfManager.self().setVariable("color", PrimChecks.math.minus_unchecked(5, 1.5));
      ...
      Errors.askNobodyCheck(world.turtles()).ask(function() {
        ...
        SelfManager.self().setVariable("in-team?", false);
        SelfManager.self().setVariable("downtime", PrimChecks.math.plus(SelfManager.self().getVariable("downtime"), 1));
      }, true);
  ...
  temp = (function() {
    try {
      ...
      for (let _index_2105_2111 = 0, _repeatcount_2105_2111 = StrictMath.floor(world.observer.getGlobal("team-size")); _index_2105_2111 < _repeatcount_2105_2111; _index_2105_2111++){
        if (Prims.gte(PrimChecks.math.randomFloat_unchecked(100), world.observer.getGlobal("p"))) {
          procedures["MAKE-NEWCOMER"]();
          newTeamMember = world.observer.getGlobal("newcomer"); letVars['newTeamMember'] = newTeamMember;
        }
        else {
          if ((Prims.lt(PrimChecks.math.randomFloat_unchecked(100), world.observer.getGlobal("q")) && PrimChecks.agentset.anyWith(world.turtles(), function() {
            return PrimChecks.math.bool('AND', SelfManager.self().getVariable("in-team?")) && PrimChecks.math.bool('AND', PrimChecks.agentset.any_unchecked(PrimChecks.agentset.with_unchecked(LinkPrims.linkNeighbors("LINKS"), function() { return PrimChecks.math.not(SelfManager.self().getVariable("in-team?")); })));
          }))) {
            newTeamMember = PrimChecks.agentset.oneOfWith(world.turtles(), function() {
              return (PrimChecks.math.not(SelfManager.self().getVariable("in-team?")) && PrimChecks.agentset.anyWith(LinkPrims.linkNeighbors("LINKS"), function() { return SelfManager.self().getVariable("in-team?"); }));
            }); letVars['newTeamMember'] = newTeamMember;
          }
          else {
            newTeamMember = PrimChecks.agentset.oneOfWith(world.turtles(), function() { return PrimChecks.math.not(SelfManager.self().getVariable("in-team?")); }); letVars['newTeamMember'] = newTeamMember;
          }
        }
        Errors.askNobodyCheck(newTeamMember).ask(function() {
          ...
          SelfManager.self().setVariable("downtime", 0);
          SelfManager.self().setVariable("size", 1.8);
          SelfManager.self().setVariable("color", (Prims.ifElseValueBooleanCheck(SelfManager.self().getVariable("incumbent?")) ? PrimChecks.math.plus_unchecked(45, 2) : PrimChecks.math.plus_unchecked(105, 1)));
        }, true);
      }
    } catch (e) {
      return Errors.stopInCommandCheck(e)
  ...
  temp = (function() {
    try {
      ...
      Errors.askNobodyCheck(PrimChecks.agentset.with_unchecked(world.links(), function() {
        return PrimChecks.math.bool('AND', PrimChecks.agentset.of(SelfManager.self().getVariable("end1"), function() { return SelfManager.self().getVariable("in-team?"); })) && PrimChecks.math.bool('AND', PrimChecks.agentset.of(SelfManager.self().getVariable("end2"), function() { return SelfManager.self().getVariable("in-team?"); }));
      })).ask(function() {
        if (SelfManager.self().getVariable("new-collaboration?")) {
          if (PrimChecks.math.bool('AND', PrimChecks.agentset.of(SelfManager.self().getVariable("end1"), function() { return SelfManager.self().getVariable("incumbent?"); })) && PrimChecks.math.bool('AND', PrimChecks.agentset.of(SelfManager.self().getVariable("end2"), function() { return SelfManager.self().getVariable("incumbent?"); }))) {
            SelfManager.self().setVariable("color", 45);
          }
          else {
            if (PrimChecks.math.bool('OR', PrimChecks.agentset.of(SelfManager.self().getVariable("end1"), function() { return SelfManager.self().getVariable("incumbent?"); })) || PrimChecks.math.bool('OR', PrimChecks.agentset.of(SelfManager.self().getVariable("end2"), function() { return SelfManager.self().getVariable("incumbent?"); }))) {
  ...
  temp = (function() {
    try {
      ...
      while (true) {
        let start = PrimChecks.agentset.oneOfWith(world.turtles(), function() { return PrimChecks.math.not(SelfManager.self().getVariable("explored?")); }); letVars['start'] = start;
  ...
  temp = (function() {
    try {
      ...
      world.observer.setGlobal("component-size", PrimChecks.math.plus(world.observer.getGlobal("component-size"), 1));
