    ...
    return workspace.rng.withClone(function() {
      return plotManager.withTemporaryContext('Link counts', undefined)(function() {
        try {
          ...
          total = PrimChecks.math.plus(PrimChecks.validator.checkArg('+', 1, total), PrimChecks.agentset.countWith(world.links(), function() { return Prims.equality(SelfManager.self().getVariable("color"), 15); })); letVars['total'] = total;
          plotManager.lowerPen();
          plotManager.plotPoint(world.ticker.tickCount(), total);
          plotManager.setCurrentPen("incumbent-incumbent");
          plotManager.raisePen();
          plotManager.plotPoint(world.ticker.tickCount(), total);
          total = PrimChecks.math.plus(PrimChecks.validator.checkArg('+', 1, total), PrimChecks.agentset.countWith(world.links(), function() { return Prims.equality(SelfManager.self().getVariable("color"), 45); })); letVars['total'] = total;
          plotManager.lowerPen();
          plotManager.plotPoint(world.ticker.tickCount(), total);
          plotManager.setCurrentPen("newcomer-incumbent");
          plotManager.raisePen();
          plotManager.plotPoint(world.ticker.tickCount(), total);
          total = PrimChecks.math.plus(PrimChecks.validator.checkArg('+', 1, total), PrimChecks.agentset.countWith(world.links(), function() { return Prims.equality(SelfManager.self().getVariable("color"), 75); })); letVars['total'] = total;
          plotManager.lowerPen();
          plotManager.plotPoint(world.ticker.tickCount(), total);
          plotManager.setCurrentPen("newcomer-newcomer");
          plotManager.raisePen();
          plotManager.plotPoint(world.ticker.tickCount(), total);
          total = PrimChecks.math.plus(PrimChecks.validator.checkArg('+', 1, total), PrimChecks.agentset.countWith(world.links(), function() { return Prims.equality(SelfManager.self().getVariable("color"), 105); })); letVars['total'] = total;
          plotManager.lowerPen();
          plotManager.plotPoint(world.ticker.tickCount(), total);
        } catch (e) {
    ...
    return workspace.rng.withClone(function() {
      return plotManager.withTemporaryContext('% of agents in the giant component', 'default')(function() {
        try {
          var reporterContext = false;
          var letVars = { };
          plotManager.plotPoint(world.ticker.tickCount(), PrimChecks.math.div(PrimChecks.validator.checkArg('/', 1, world.observer.getGlobal("giant-component-size")), PrimChecks.agentset.count(world.turtles())));
        } catch (e) {
          return Errors.stopInCommandCheck(e)
        };
      });
    ...
    return workspace.rng.withClone(function() {
      return plotManager.withTemporaryContext('Average component size', 'default')(function() {
        try {
          var reporterContext = false;
          var letVars = { };
          plotManager.plotPoint(world.ticker.tickCount(), PrimChecks.list.mean(PrimChecks.validator.checkArg('MEAN', 8, world.observer.getGlobal("components"))));
        } catch (e) {
          return Errors.stopInCommandCheck(e)
        };
      });
...
var procedures = (function() {
  ...
  temp = (function() {
    try {
      ...
      Errors.askNobodyCheck(world.turtles()).ask(function() {
        SelfManager.self().setVariable("heading", PrimChecks.math.mult(PrimChecks.math.div(360, PrimChecks.validator.checkArg('/', 1, world.observer.getGlobal("team-size"))), PrimChecks.validator.checkArg('*', 1, SelfManager.self().getVariable("who"))));
  ...
  temp = (function() {
    try {
      ...
      Errors.askNobodyCheck(world.turtles()).ask(function() {
        ...
        SelfManager.self().setVariable("in-team?", false);
        SelfManager.self().setVariable("downtime", PrimChecks.math.plus(PrimChecks.validator.checkArg('+', 1, SelfManager.self().getVariable("downtime")), 1));
      }, true);
      ...
      for (let _index_2105_2111 = 0, _repeatcount_2105_2111 = StrictMath.floor(world.observer.getGlobal("team-size")); _index_2105_2111 < _repeatcount_2105_2111; _index_2105_2111++){
        if (Prims.gte(PrimChecks.math.randomFloat(100), world.observer.getGlobal("p"))) {
          ...
          if ((Prims.lt(PrimChecks.math.randomFloat(100), world.observer.getGlobal("q")) && PrimChecks.agentset.anyWith(world.turtles(), function() {
            return (PrimChecks.validator.checkArg('AND', 2, SelfManager.self().getVariable("in-team?")) && PrimChecks.agentset.any(PrimChecks.agentset.with(LinkPrims.linkNeighbors("LINKS"), function() {
              return PrimChecks.math.not(PrimChecks.validator.checkArg('NOT', 2, SelfManager.self().getVariable("in-team?")));
            })));
          }))) {
            newTeamMember = PrimChecks.agentset.oneOfWith(world.turtles(), function() {
              return (PrimChecks.math.not(PrimChecks.validator.checkArg('NOT', 2, SelfManager.self().getVariable("in-team?"))) && PrimChecks.agentset.anyWith(LinkPrims.linkNeighbors("LINKS"), function() { return SelfManager.self().getVariable("in-team?"); }));
        ...
        Errors.askNobodyCheck(newTeamMember).ask(function() {
          ...
          SelfManager.self().setVariable("downtime", 0);
          SelfManager.self().setVariable("size", 1.8);
          SelfManager.self().setVariable("color", (Prims.ifElseValueBooleanCheck(SelfManager.self().getVariable("incumbent?")) ? PrimChecks.math.plus(45, 2) : PrimChecks.math.plus(105, 1)));
        }, true);
      }
    } catch (e) {
      return Errors.stopInCommandCheck(e)
  ...
  temp = (function() {
    try {
      var reporterContext = false;
      var letVars = { };
      Errors.askNobodyCheck(PrimChecks.agentset.with(world.links(), function() {
        return (PrimChecks.validator.checkArg('AND', 2, PrimChecks.agentset.of(PrimChecks.validator.checkArg('OF', 1904, SelfManager.self().getVariable("end1")), function() { return SelfManager.self().getVariable("in-team?"); })) && PrimChecks.validator.checkArg('AND', 2, PrimChecks.agentset.of(PrimChecks.validator.checkArg('OF', 1904, SelfManager.self().getVariable("end2")), function() { return SelfManager.self().getVariable("in-team?"); })));
      })).ask(function() {
        if (SelfManager.self().getVariable("new-collaboration?")) {
          if ((PrimChecks.validator.checkArg('AND', 2, PrimChecks.agentset.of(PrimChecks.validator.checkArg('OF', 1904, SelfManager.self().getVariable("end1")), function() { return SelfManager.self().getVariable("incumbent?"); })) && PrimChecks.validator.checkArg('AND', 2, PrimChecks.agentset.of(PrimChecks.validator.checkArg('OF', 1904, SelfManager.self().getVariable("end2")), function() { return SelfManager.self().getVariable("incumbent?"); })))) {
            SelfManager.self().setVariable("color", 45);
          }
          else {
            if ((PrimChecks.validator.checkArg('OR', 2, PrimChecks.agentset.of(PrimChecks.validator.checkArg('OF', 1904, SelfManager.self().getVariable("end1")), function() { return SelfManager.self().getVariable("incumbent?"); })) || PrimChecks.validator.checkArg('OR', 2, PrimChecks.agentset.of(PrimChecks.validator.checkArg('OF', 1904, SelfManager.self().getVariable("end2")), function() { return SelfManager.self().getVariable("incumbent?"); })))) {
  ...
  temp = (function() {
    try {
      ...
      while (true) {
        let start = PrimChecks.agentset.oneOfWith(world.turtles(), function() {
          return PrimChecks.math.not(PrimChecks.validator.checkArg('NOT', 2, SelfManager.self().getVariable("explored?")));
        }); letVars['start'] = start;
  ...
  temp = (function() {
    try {
      ...
      world.observer.setGlobal("component-size", PrimChecks.math.plus(PrimChecks.validator.checkArg('+', 1, world.observer.getGlobal("component-size")), 1));
