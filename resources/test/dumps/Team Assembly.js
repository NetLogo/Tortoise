...
function makeNewcomer() {
  AgentSet.ask(world.createTurtles(1, ""), true, function() {
    ...
    Globals.setGlobal(6, AgentSet.self());
...
function setup() {
  ...
  AgentSet.ask(world.turtles(), true, function() {
    AgentSet.setTurtleVariable(2, ((360 / Globals.getGlobal(3)) * AgentSet.getTurtleVariable(0)));
    Prims.fd(1.75);
    AgentSet.setTurtleVariable(14, false);
  });
...
function go() {
  ...
  Call(colorCollaborations);
  AgentSet.ask(world.turtles(), true, function() {
    if (Prims.gt(AgentSet.getTurtleVariable(15), Globals.getGlobal(5))) {
      AgentSet.die();
    }
    AgentSet.setTurtleVariable(14, false);
...
function pickTeamMembers() {
  ...
  Prims.repeat(Globals.getGlobal(3), function () {
    if (Prims.gte(Prims.randomFloat(100), Globals.getGlobal(1))) {
      Call(makeNewcomer);
      newTeamMember = Globals.getGlobal(6);
    }
    else {
      if ((Prims.lt(Prims.randomFloat(100), Globals.getGlobal(2)) && AgentSet.any(AgentSet.agentFilter(world.turtles(), function() {
...
function tieCollaborators() {
  AgentSet.ask(AgentSet.agentFilter(world.turtles(), function() {
    ...
    AgentSet.ask(AgentSet.createLinksWith(AgentSet.other(AgentSet.agentFilter(world.turtles(), function() {
      return AgentSet.getTurtleVariable(14);
    }))), true, function() {
...
function findAllComponents() {
  ...
  while (true) {
    ...
    Globals.setGlobal(7, 0);
    AgentSet.ask(start, true, function() {
      Call(explore);
    });
    if (Prims.gt(Globals.getGlobal(7), Globals.getGlobal(8))) {
      Globals.setGlobal(8, Globals.getGlobal(7));
    }
    Globals.setGlobal(9, Prims.lput(Globals.getGlobal(7), Globals.getGlobal(9)));
  };
}
function explore() {
  ...
  Globals.setGlobal(7, (Globals.getGlobal(7) + 1));
  AgentSet.ask(AgentSet.linkNeighbors(false, false), true, function() {
    Call(explore);
  });
}
Globals.setGlobal(0, true);
Globals.setGlobal(1, 40);
Globals.setGlobal(2, 65);
Globals.setGlobal(3, 4);
Globals.setGlobal(4, true);
Globals.setGlobal(5, 40);
