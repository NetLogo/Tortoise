...
function setup() {
  ...
  AgentSet.ask(world.turtles(), true, function() {
    AgentSet.setTurtleVariable(2, ((360 / world.observer.getGlobal('team-size')) * AgentSet.getTurtleVariable(0)));
    Prims.fd(1.75);
    AgentSet.setTurtleVariable(14, false);
  });
...
function go() {
  ...
  Call(colorCollaborations);
  AgentSet.ask(world.turtles(), true, function() {
    if (Prims.gt(AgentSet.getTurtleVariable(15), world.observer.getGlobal('max-downtime'))) {
      AgentSet.die();
    }
    AgentSet.setTurtleVariable(14, false);
