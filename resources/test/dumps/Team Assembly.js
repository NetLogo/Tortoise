    ...
    return workspace.rng.withClone(function() {
      return plotManager.withTemporaryContext('Link counts', undefined)(function() {
        try {
          ...
          total = (total + world.links()._optimalCountWith(function() { return Prims.equality(SelfManager.self().getVariable("color"), 15); })); letVars['total'] = total;
          plotManager.lowerPen();
          plotManager.plotPoint(world.ticker.tickCount(), total);
          plotManager.setCurrentPen("incumbent-incumbent");
          plotManager.raisePen();
          plotManager.plotPoint(world.ticker.tickCount(), total);
          total = (total + world.links()._optimalCountWith(function() { return Prims.equality(SelfManager.self().getVariable("color"), 45); })); letVars['total'] = total;
          plotManager.lowerPen();
          plotManager.plotPoint(world.ticker.tickCount(), total);
          plotManager.setCurrentPen("newcomer-incumbent");
          plotManager.raisePen();
          plotManager.plotPoint(world.ticker.tickCount(), total);
          total = (total + world.links()._optimalCountWith(function() { return Prims.equality(SelfManager.self().getVariable("color"), 75); })); letVars['total'] = total;
          plotManager.lowerPen();
          plotManager.plotPoint(world.ticker.tickCount(), total);
          plotManager.setCurrentPen("newcomer-newcomer");
          plotManager.raisePen();
          plotManager.plotPoint(world.ticker.tickCount(), total);
          total = (total + world.links()._optimalCountWith(function() { return Prims.equality(SelfManager.self().getVariable("color"), 105); })); letVars['total'] = total;
          plotManager.lowerPen();
          plotManager.plotPoint(world.ticker.tickCount(), total);
        } catch (e) {
...
var procedures = (function() {
  ...
  temp = (function() {
    try {
      var reporterContext = false;
      var letVars = { };
      Errors.askNobodyCheck(world.turtles().agentFilter(function() { return SelfManager.self().getVariable("in-team?"); })).ask(function() {
  ...
  temp = (function() {
    try {
      var reporterContext = false;
      var letVars = { };
      Errors.askNobodyCheck(world.links().agentFilter(function() {
