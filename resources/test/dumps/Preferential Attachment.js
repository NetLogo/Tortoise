    ...
    return workspace.rng.withClone(function() {
      return plotManager.withTemporaryContext('Degree Distribution (log-log)', 'default')(function() {
        try {
          ...
          let maxDegree = PrimChecks.list.max(PrimChecks.validator.checkArg('MAX', 8, PrimChecks.agentset.of(world.turtles(), function() { return PrimChecks.agentset.count(LinkPrims.linkNeighbors("LINKS")); }))); letVars['maxDegree'] = maxDegree;
          plotManager.resetPen();
          let degree = 1; letVars['degree'] = degree;
          while (Prims.lte(degree, maxDegree)) {
            let matches = PrimChecks.agentset.with(world.turtles(), function() { return Prims.equality(PrimChecks.agentset.count(LinkPrims.linkNeighbors("LINKS")), degree); }); letVars['matches'] = matches;
            if (PrimChecks.agentset.any(PrimChecks.validator.checkArg('ANY?', 112, matches))) {
              plotManager.plotPoint(PrimChecks.math.log(PrimChecks.validator.checkArg('LOG', 1, degree), 10), PrimChecks.math.log(PrimChecks.agentset.count(PrimChecks.validator.checkArg('COUNT', 112, matches)), 10));
            }
            degree = PrimChecks.math.plus(PrimChecks.validator.checkArg('+', 1, degree), 1); letVars['degree'] = degree;
          }
        } catch (e) {
    ...
    return workspace.rng.withClone(function() {
      return plotManager.withTemporaryContext('Degree Distribution', 'default')(function() {
        try {
          ...
          let maxDegree = PrimChecks.list.max(PrimChecks.validator.checkArg('MAX', 8, PrimChecks.agentset.of(world.turtles(), function() { return PrimChecks.agentset.count(LinkPrims.linkNeighbors("LINKS")); }))); letVars['maxDegree'] = maxDegree;
          plotManager.resetPen();
          plotManager.setXRange(1, PrimChecks.math.plus(PrimChecks.validator.checkArg('+', 1, maxDegree), 1));
...
var procedures = (function() {
  ...
  temp = (function() {
    try {
      var reporterContext = false;
      var letVars = { };
      Errors.askNobodyCheck(world.links()).ask(function() { SelfManager.self().setVariable("color", 5); }, true);
  ...
  temp = (function() {
    try {
      ...
      return PrimChecks.agentset.of(PrimChecks.validator.checkArg('OF', 1904, PrimChecks.list.oneOf(world.links())), function() { return PrimChecks.list.oneOf(SelfManager.self().bothEnds()); });
  ...
  temp = (function() {
    try {
      var reporterContext = false;
      var letVars = { };
      if (PrimChecks.agentset.all(world.turtles(), function() { return Prims.lte(SelfManager.self().getVariable("size"), 1); })) {
        Errors.askNobodyCheck(world.turtles()).ask(function() {
          SelfManager.self().setVariable("size", PrimChecks.math.sqrt(PrimChecks.agentset.count(LinkPrims.linkNeighbors("LINKS"))));
        }, true);
      }
      else {
        Errors.askNobodyCheck(world.turtles()).ask(function() { SelfManager.self().setVariable("size", 1); }, true);
      }
    } catch (e) {
      return Errors.stopInCommandCheck(e)
    }
  ...
  temp = (function() {
    try {
      ...
      for (let _index_2025_2031 = 0, _repeatcount_2025_2031 = StrictMath.floor(3); _index_2025_2031 < _repeatcount_2025_2031; _index_2025_2031++){
        let factor = PrimChecks.math.sqrt(PrimChecks.agentset.count(world.turtles())); letVars['factor'] = factor;
      ...
      let xOffset = PrimChecks.math.plus(PrimChecks.list.max(PrimChecks.validator.checkArg('MAX', 8, PrimChecks.agentset.of(world.turtles(), function() { return SelfManager.self().getVariable("xcor"); }))), PrimChecks.list.min(PrimChecks.validator.checkArg('MIN', 8, PrimChecks.agentset.of(world.turtles(), function() { return SelfManager.self().getVariable("xcor"); })))); letVars['xOffset'] = xOffset;
      let yOffset = PrimChecks.math.plus(PrimChecks.list.max(PrimChecks.validator.checkArg('MAX', 8, PrimChecks.agentset.of(world.turtles(), function() { return SelfManager.self().getVariable("ycor"); }))), PrimChecks.list.min(PrimChecks.validator.checkArg('MIN', 8, PrimChecks.agentset.of(world.turtles(), function() { return SelfManager.self().getVariable("ycor"); })))); letVars['yOffset'] = yOffset;
      ...
      Errors.askNobodyCheck(world.turtles()).ask(function() {
        SelfManager.self().setXY(PrimChecks.math.minus(PrimChecks.validator.checkArg('-', 1, SelfManager.self().getVariable("xcor")), PrimChecks.math.div(PrimChecks.validator.checkArg('/', 1, xOffset), 2)), PrimChecks.math.minus(PrimChecks.validator.checkArg('-', 1, SelfManager.self().getVariable("ycor")), PrimChecks.math.div(PrimChecks.validator.checkArg('/', 1, yOffset), 2)));
      }, true);
    } catch (e) {
      return Errors.stopInCommandCheck(e)
    }
  ...
  temp = (function(number, limit) {
    try {
      ...
      if (Prims.lt(number, PrimChecks.math.unaryminus(PrimChecks.validator.checkArg('-', 1, limit)))) {
        Errors.reportInContextCheck(reporterContext);
        return PrimChecks.math.unaryminus(PrimChecks.validator.checkArg('-', 1, limit));
      }
      Errors.reportInContextCheck(reporterContext);
      return number;
      Errors.missingReport();
