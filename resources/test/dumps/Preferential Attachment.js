    ...
    return workspace.rng.withClone(function() {
      return plotManager.withTemporaryContext('Degree Distribution (log-log)', 'default')(function() {
        try {
          ...
          let maxDegree = PrimChecks.list.max(world.turtles().projectionBy(function() { return LinkPrims.linkNeighbors("LINKS").size(); })); letVars['maxDegree'] = maxDegree;
          ...
          while (Prims.lte(degree, maxDegree)) {
            ...
            if (!matches.isEmpty()) {
              plotManager.plotPoint(PrimChecks.math.log(degree, 10), PrimChecks.math.log(matches.size(), 10));
            }
...
var procedures = (function() {
  ...
  temp = (function() {
    try {
      ...
      return PrimChecks.list.oneOf(world.links()).projectionBy(function() { return PrimChecks.list.oneOf(SelfManager.self().bothEnds()); });
    ...
    try {
      ...
      if (world.turtles().agentAll(function() { return Prims.lte(SelfManager.self().getVariable("size"), 1); })) {
        Errors.askNobodyCheck(world.turtles()).ask(function() {
          SelfManager.self().setVariable("size", PrimChecks.math.sqrt(LinkPrims.linkNeighbors("LINKS").size()));
        }, true);
      }
      else {
  ...
  temp = (function() {
    try {
      ...
      for (let _index_2025_2031 = 0, _repeatcount_2025_2031 = StrictMath.floor(3); _index_2025_2031 < _repeatcount_2025_2031; _index_2025_2031++){
        let factor = PrimChecks.math.sqrt(world.turtles().size()); letVars['factor'] = factor;
      ...
      let xOffset = (PrimChecks.list.max(world.turtles().projectionBy(function() { return SelfManager.self().getVariable("xcor"); })) + PrimChecks.list.min(world.turtles().projectionBy(function() { return SelfManager.self().getVariable("xcor"); }))); letVars['xOffset'] = xOffset;
      let yOffset = (PrimChecks.list.max(world.turtles().projectionBy(function() { return SelfManager.self().getVariable("ycor"); })) + PrimChecks.list.min(world.turtles().projectionBy(function() { return SelfManager.self().getVariable("ycor"); }))); letVars['yOffset'] = yOffset;
