    ...
    return workspace.rng.withClone(function() {
      return plotManager.withTemporaryContext('Degree Distribution (log-log)', 'default')(function() {
        try {
          ...
          var letVars = { };
          if (PrimChecks.math.not(world.observer.getGlobal("plot?"))) {
            throw new Exception.StopInterrupt;
          }
          let maxDegree = PrimChecks.list.max(PrimChecks.agentset.of_unchecked(world.turtles(), function() { return PrimChecks.agentset.count_unchecked(LinkPrims.linkNeighbors("LINKS")); })); letVars['maxDegree'] = maxDegree;
          plotManager.resetPen();
          let degree = 1; letVars['degree'] = degree;
          while (Prims.lte(degree, maxDegree)) {
            let matches = PrimChecks.agentset.with_unchecked(world.turtles(), function() {
              return Prims.equality(PrimChecks.agentset.count_unchecked(LinkPrims.linkNeighbors("LINKS")), degree);
            }); letVars['matches'] = matches;
            if (PrimChecks.agentset.any(matches)) {
              plotManager.plotPoint(PrimChecks.math.log(degree, 10), PrimChecks.math.log_unchecked(PrimChecks.agentset.count(matches), 10));
            }
            degree = PrimChecks.math.plus(degree, 1); letVars['degree'] = degree;
          }
        } catch (e) {
    ...
    return workspace.rng.withClone(function() {
      return plotManager.withTemporaryContext('Degree Distribution', 'default')(function() {
        try {
          ...
          let maxDegree = PrimChecks.list.max(PrimChecks.agentset.of_unchecked(world.turtles(), function() { return PrimChecks.agentset.count_unchecked(LinkPrims.linkNeighbors("LINKS")); })); letVars['maxDegree'] = maxDegree;
          plotManager.resetPen();
          plotManager.setXRange(1, PrimChecks.math.plus(maxDegree, 1));
          plotManager.drawHistogramFrom(PrimChecks.agentset.of_unchecked(world.turtles(), function() { return PrimChecks.agentset.count_unchecked(LinkPrims.linkNeighbors("LINKS")); }));
        } catch (e) {
          return Errors.stopInCommandCheck(e)
        };
      });
...
var procedures = (function() {
  ...
  temp = (function() {
    try {
      ...
      return PrimChecks.agentset.of(PrimChecks.list.oneOf_unchecked(world.links()), function() { return PrimChecks.list.oneOf_unchecked(SelfManager.self().bothEnds()); });
  ...
  temp = (function() {
    try {
      var reporterContext = false;
      var letVars = { };
      if (PrimChecks.agentset.all_unchecked(world.turtles(), function() { return Prims.lte(SelfManager.self().getVariable("size"), 1); })) {
        Errors.askNobodyCheck(world.turtles()).ask(function() {
          SelfManager.self().setVariable("size", PrimChecks.math.sqrt_unchecked(PrimChecks.agentset.count_unchecked(LinkPrims.linkNeighbors("LINKS"))));
        }, true);
      }
      else {
  ...
  temp = (function() {
    try {
      ...
      for (let _index_2025_2031 = 0, _repeatcount_2025_2031 = StrictMath.floor(3); _index_2025_2031 < _repeatcount_2025_2031; _index_2025_2031++){
        let factor = PrimChecks.math.sqrt_unchecked(PrimChecks.agentset.count_unchecked(world.turtles())); letVars['factor'] = factor;
        LayoutManager.layoutSpring(world.turtles(), world.links(), PrimChecks.math.div(1, factor), PrimChecks.math.div(7, factor), PrimChecks.math.div(1, factor));
        notImplemented('display', undefined)();
      }
      let xOffset = PrimChecks.math.plus_unchecked(PrimChecks.list.max(PrimChecks.agentset.of_unchecked(world.turtles(), function() { return SelfManager.self().getVariable("xcor"); })), PrimChecks.list.min(PrimChecks.agentset.of_unchecked(world.turtles(), function() { return SelfManager.self().getVariable("xcor"); }))); letVars['xOffset'] = xOffset;
      let yOffset = PrimChecks.math.plus_unchecked(PrimChecks.list.max(PrimChecks.agentset.of_unchecked(world.turtles(), function() { return SelfManager.self().getVariable("ycor"); })), PrimChecks.list.min(PrimChecks.agentset.of_unchecked(world.turtles(), function() { return SelfManager.self().getVariable("ycor"); }))); letVars['yOffset'] = yOffset;
      ...
      Errors.askNobodyCheck(world.turtles()).ask(function() {
        SelfManager.self().setXY(PrimChecks.math.minus(SelfManager.self().getVariable("xcor"), PrimChecks.math.div(xOffset, 2)), PrimChecks.math.minus(SelfManager.self().getVariable("ycor"), PrimChecks.math.div(yOffset, 2)));
      }, true);
    } catch (e) {
      return Errors.stopInCommandCheck(e)
    }
  ...
  temp = (function(number, limit) {
    try {
      ...
      if (Prims.lt(number, PrimChecks.math.unaryminus(limit))) {
        Errors.reportInContextCheck(reporterContext);
        return PrimChecks.math.unaryminus(limit);
      }
      Errors.reportInContextCheck(reporterContext);
      return number;
      Errors.missingReport();
