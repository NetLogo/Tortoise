        ...
        try {
          ...
          while (Prims.lte(degree, maxDegree)) {
            ...
            if (PrimChecks.agentset.any(matches)) {
              plotManager.plotPoint(PrimChecks.math.log(degree, 10), PrimChecks.math.log(PrimChecks.agentset.count(matches), 10));
            }
...
var procedures = (function() {
  ...
  temp = (function() {
    try {
      ...
      return PrimChecks.agentset.of(PrimChecks.list.oneOf(world.links()), function() { return PrimChecks.list.oneOf(SelfManager.self().bothEnds()); });
    ...
    try {
      ...
      if (PrimChecks.agentset.all_unchecked(world.turtles(), function() { return Prims.lte(SelfManager.self().getVariable("size"), 1); })) {
        Errors.askNobodyCheck(world.turtles()).ask(function() {
          SelfManager.self().setVariable("size", PrimChecks.math.sqrt(PrimChecks.agentset.count_unchecked(LinkPrims.linkNeighbors("LINKS"))));
        }, true);
      }
      else {
  ...
  temp = (function() {
    try {
      ...
      for (let _index_2025_2031 = 0, _repeatcount_2025_2031 = StrictMath.floor(3); _index_2025_2031 < _repeatcount_2025_2031; _index_2025_2031++){
        let factor = PrimChecks.math.sqrt(PrimChecks.agentset.count_unchecked(world.turtles())); letVars['factor'] = factor;
      ...
      let xOffset = (PrimChecks.list.max(PrimChecks.agentset.of_unchecked(world.turtles(), function() { return SelfManager.self().getVariable("xcor"); })) + PrimChecks.list.min(PrimChecks.agentset.of_unchecked(world.turtles(), function() { return SelfManager.self().getVariable("xcor"); }))); letVars['xOffset'] = xOffset;
      let yOffset = (PrimChecks.list.max(PrimChecks.agentset.of_unchecked(world.turtles(), function() { return SelfManager.self().getVariable("ycor"); })) + PrimChecks.list.min(PrimChecks.agentset.of_unchecked(world.turtles(), function() { return SelfManager.self().getVariable("ycor"); }))); letVars['yOffset'] = yOffset;
      ...
      Errors.askNobodyCheck(world.turtles()).ask(function() {
        SelfManager.self().setXY((SelfManager.self().getVariable("xcor") - PrimChecks.math.div(xOffset, 2)), (SelfManager.self().getVariable("ycor") - PrimChecks.math.div(yOffset, 2)));
      }, true);
    } catch (e) {
      return Errors.stopInCommandCheck(e)
    }
