...
object WidgetToJsonTest extends TestSuite {
    ...
    "test serialization, deserialization, round-tripping"-{
      (testJsons zip testWidgets).foreach {
        case ((name, json), (_, widget)) =>
          ...
          assert(readWidget.isRight)
          assert(readWidget.right.get == widget)

          val roundTripSerialized = JsonLibrary.toTortoise(JsonLibrary.toNative(json)).asInstanceOf[JsObject]
          val readObject = WidgetToJson.read(roundTripSerialized)
          assert(readObject.isRight)
          assert(readObject.right.get.toJsonObj == json)
          assert(WidgetToJson.read(widget.toJsonObj.asInstanceOf[JsObject]).right.get == widget)
      }
    }

  "fails to read without a type"-{
    assert(Left("no conversion available") == WidgetToJson.read(JsObject(fields())))
  }
  ...
  def testWidgets: Map[String, Widget] = Map(
    ...
    "chooser" -> {
      val choices = List(
        Chooseable(10.5: java.lang.Double),
        Chooseable("def"),
    ...
    "view"    ->View(1, 2, 3, 4, 25.0, 8, true, false, 10, 9, 8, 7, UpdateMode.Continuous, false, "tocks", 50.5)
    )
  }
}
