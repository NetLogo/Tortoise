...
lazy val netLogoWeb: Project = (project in file("netlogo-web")).
  dependsOn(tortoise % "compile-internal->compile;test-internal->test").
  ...
  settings(
    ...
    compile <<= (compile in Compile) dependsOn(cleanGeneratedSources, managedResources in Compile, clean in tortoiseJs, fullOptJS in Compile in tortoiseJs),
    compile <<= (compile in Test) dependsOn(cleanGeneratedSources, managedResources in Compile, clean in tortoiseJs, fullOptJS in Compile in tortoiseJs),
    name := "NetLogoWebJS",
    libraryDependencies ++= Seq(
