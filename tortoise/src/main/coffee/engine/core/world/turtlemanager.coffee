...
ColorModel = require('util/colormodel')
...
module.exports =
  class TurtleManager
    ...
    _createTurtle: (color, heading, xcor, ycor, breed, label, lcolor, isHidden, size, shape, genPenManager) =>
      id     = @_idManager.next()
      turtle = new Turtle(@_world, id, @_updater.updated, @_updater.registerDeadTurtle, @_createTurtle, @_removeTurtle, color, heading, xcor, ycor, breed, label, lcolor, isHidden, size, shape, genPenManager)
