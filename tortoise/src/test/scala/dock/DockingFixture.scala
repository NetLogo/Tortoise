...
class DockingFixture(name: String, nashorn: Nashorn) extends Fixture(name) {
  ...
  private def updatedJsonModels(expectedJson: String, actualJson: String) : (String, String) = {
    nashorn.eval(s"""expectedUpdates = JSON.parse("${expectedJson.replaceAll("\"", "\\\\\"")}")""")
    nashorn.eval(s"""actualUpdates   = JSON.parse("${actualJson.replaceAll("\"", "\\\\\"")}")""")
  ...
  def open(path: String, dimensions: Option[(Int, Int, Int, Int)]) {
    require(!opened)
    super.open(path)
    val model = ModelReader.parseModel(api.FileIO.file2String(path), workspace.parser)
